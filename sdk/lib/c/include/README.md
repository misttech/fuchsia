...
plus some Fuchsia-specific (Zircon) extensions.  (A few of the standard C
headers are provided by the C compiler instead and are not here.)
...
Most of the truly public API headers do not exist directly as `.h` files in
...
emitted by the build into the `gen/third_party/llvm-libc/src/include` and
`gen/sdk/lib/c/include` subdirectories of the build directory.  These are used
along with those `include` source directories when building libc.
...
different version.  If mismatches arise during development, it may be necessary
to set `update_goldens=true` and repeat the build multiple times with `-k0` to
...
### Golden files for generated headers

The [`gen' subdirectory here](gen) contains header files that are checked into
the source tree, but maintained entirely by automatic processes that are
triggered manually.

Having these in the source tree accomplishes two things:
 1. After just `gn gen`, enough information can be gleaned to fully plumb
    compilations using these headers or to construct a sysroot containing them.
    No Ninja steps need to be run.
 2. The verbatim files that will be installed appear in the diffs for a change.
    When changes to the generator or the YAML data are made, it's not obvious
    what their exact effects on the output will be.  Moreover, even those
    changes might be in [llvm-libc] and not directly visible a the change.
    Code and API reviewers can respond to the material outcome of the change,
    and not only its indirect implementation details.

[`gen`](gen) contains all the header files generated from `.yaml` files, both
those in [llvm-libc](/third_party/llvm-libc/src/include) and those maintained
[right here](zircon).  It also contains additional "redirect" headers that are
generated for each hand-maintained header file (`llvm-libc-types/*.h` et al)
that allow the [`gen`](gen) directory to be used in the compiler search path.
These are needed because libc headers referring to each other always use
`#include "..."` syntax with precise relative paths to ensure that the matched
set is always used in the final install situation (like the SDK's sysroot).  So
to use them in place in the source tree, their as-if-installed relative paths
must resolve from their source-tree locations too.  The redirect headers ensure
that.  So they're generated and checked in as part of the same build logic that
handle the YAML-based header generation.

The build always regenerates all these files from their sources of truth in GN
and YAML files.  Those generated files are used for building libc itself, as
described above.  The [`gen`](gen) files are checked against, and/or updated
from, those via the `golden_files()` mechanism.

### Landing header changes

Any kind of change to installed headers requires [API review](#sysroot_api)
procedures.  The same [procedure described above](#sysroot_api) will also
handles updating [`gen`](gen).  All the changes to the implementation source of
truth files, [`gen`](gen) and `.api` files, must land together in one commit.

Certain kinds of changes to [`libc_headers.gni`](libc_headers.gni) or to YAML
or GN files may require repeating the build a few times (perhaps with `-k0`) to
reach a stable state with interdependent golden-file updates.  So be sure to be
all the way through a complete build (of at least `sysroot_sdk`) before commit.

#### Landing llvm-libc header updates

Any upstream llvm-libc change to YAML files, hand-maintained header files, or
to the `hdrgen` tool, may constitute a change to installed header files.  These
are no different, except that the "implementation source of truth" part of the
commit is just a revision hash change in [manifests].

These changes thus will also be [manual rolls].  Whenever possible, commit an
isolated change that is just the manual roll along with the golden file updates
and no other changes that aren't strictly necessary to let the roll land.
