...
btf::setup_fx() {
  ...
  # Fake a build directory with the minimal content so that fx works
...
btf::make_installed_hosttools_mock() {
  ...
  btf::_make_buildtool_mock "${REL_BUILD_DIR:-out/default}/host-tools/${hosttool_name}"
}

btf::make_hosttools_mock() {
  local hosttool_name="$1"
  btf::_make_buildtool_mock "${REL_BUILD_DIR:-out/default}/host_x64/${hosttool_name}"
}
