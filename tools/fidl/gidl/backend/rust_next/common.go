...
func visit(value ir.Value, decl mixer.Declaration) string {
	...
	case int64, uint64, float64:
		...
		case *mixer.EnumDecl:
			primitive := visit(value, &decl.Underlying)
			if decl.IsFlexible() {
				return fmt.Sprintf("%s::from(%v)", declName(decl), primitive)
			}
			return fmt.Sprintf("%s::try_from(%v).unwrap()", declName(decl), primitive)
		}
	case ir.RawFloat:
...
func onDecodedRecord(value ir.DecodedRecord, decl mixer.RecordDeclaration) string {
	if value.Encoding.WireFormat != ir.V2WireFormat {
		panic("Rust Next backend only supports V2 in decode() function")
	}
	name := declName(decl)
	...
	return fmt.Sprintf("decode_value::<%s>(&%s, &mut %s)", name, bytes, handles)
}
