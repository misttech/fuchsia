...
(
  cd "${DEST_DIR}"
  cat <<EOF
# Copyright $(date +%Y) The Fuchsia Authors. All rights reserved.
# Use of this source code is governed by a BSD-style license that can be
# found in the LICENSE file.

# Auto-generated by tools/build/scripts/extract_protobuf_py3_wheel.sh. DO NOT EDIT!

import("//build/python/python_library.gni")

python_library("protobuf-py3") {
  library_name = "google"
  source_root = "."

  # Skip MyPy type-checking on this third-party prebuilt library itself because
  # it lacks PEP-484 annotations and PEP-561 type stubs, which would otherwise
  # trigger numerous type-checking errors. This does not impact MyPy checks
  # on downstream targets, which can continue to safely import it.
  enable_mypy = false
  sources = [
EOF
  # List all extracted python source files
  find . -type f -name "*.py" | sed -E 's|^\./||' | sort | while read -r pyfile; do
    echo "    \"${pyfile}\","
  done
  cat <<EOF
  ]
