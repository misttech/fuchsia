...
// ExportLastBuildDebugSymbols exports the debug symbols generated by the last build
// invocation to a new directory.
func (c BuildAPIClient) ExportLastBuildDebugSymbols(output_dir string, withBreakpadSymbols, withGsymSymbols bool) error {
	cmdArgs := []string{"--build-dir", c.buildDir, "export_last_build_debug_symbols", "--output-dir", output_dir}
	if withBreakpadSymbols {
		cmdArgs = append(cmdArgs, "--with-breakpad-symbols")
	}
	if withGsymSymbols {
		cmdArgs = append(cmdArgs, "--with-gsym-symbols")
	}
	cmd := exec.Command(c.toolPath, cmdArgs...)
	cmd.Stderr = os.Stderr
	_, err := cmd.Output()
	return err
}
