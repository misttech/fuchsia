...
import (
	...
	"go.fuchsia.dev/fuchsia/tools/dart_test_parser"
...
func usage() {
	fmt.Printf(`dart_test_parser [dart test command]

Reads stdout from the dart test command, and writes a JSON formatted summary to stdout
...
func mainImpl() error {
	...
	if outputSummaryPath := os.Getenv(constants.TestOutputSummaryPathEnvKey); outputSummaryPath != "" {
		cases := dart_test_parser.Parse(stdoutForParsing.Bytes())
