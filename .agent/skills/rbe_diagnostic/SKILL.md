...
* Check `out/_build_logs/<config>/build.<timestamp>.<id>/reproxy_logs/`.
    * `<config>` is your build directory's basename, which can be found by `fx
      use` or looking at `.fx-build-dir`.
    * `<id>` is a random suffix.
    * Find the most recent `build.*` directory that corresponds to the failed
      build.
* Look for `reproxy.INFO`, `scandeps_server.INFO`, and `.rrpl` (reclient reproxy
  proto log) files.
* If `FX_BUILD_RBE_STATS=1` was set, the log directory is printed in the build
  summary. It is generally useful to set this environment variable while
...
* **Missing Remote Inputs**: Calculation of remote inputs is different for each
  language/tool.
    * C++: Uses a lightweight `build/rbe/reclient_cxx.sh` (mostly forward
      directly to `rewrapper`, using its built-in InputProcessor), unless
      additional features are needed from `cxx_remote_wrapper.py`.
