...
as well as extracting target device identifiers directly from Honeydew device objects.
"""
...
def get_dut_serial(dut: Any) -> Optional[str]:
    """Extract serial number or hardware identifier from a Honeydew FuchsiaDevice.
        ...
        if isinstance(sn, str) and sn:
            ...
            try:
                ...
                if loop and loop.is_running():
                    ...
                    try:
                        res = pool.submit(_call_worker).result(timeout=5.0)
                    finally:
                ...
                _LOGGER.debug(f"Failed calling dut.serial_number(): {e}")

    # 2. Check _device_info on FuchsiaDevice (ensure single serial, not multi-device string).
    ...
    # 3. Check ffx target information if available (supports dict and TargetInfoData)
    try:
        ...
        _LOGGER.debug(f"Failed querying ffx target info for serial: {e}")
    ...
    try:
        ...
        _LOGGER.debug(f"Failed querying fastboot serial: {e}")
...
def get_usb_config(dut: Any) -> str:
    ...
    try:
        ...
        _LOGGER.debug(f"Failed querying usb config via SSH ({e})")

    # 2. Fallback: Serial command if target is offline or network is disconnected
...
def normalize_usb_functions(functions: List[str]) -> List[str]:
    """Normalize a list of function names, deduplicating functions preserving order.

    Args:
        functions: List of USB peripheral function names (e.g. ['cdc', 'vsock']).
...
def parse_usb_config_functions(config_str: str) -> List[str]:
    ...
    try:
        # usb-cli get-config outputs JSON like {"functions": ["cdc", "adb", "vsock"]}
        ...
        _LOGGER.debug(
            f"Failed to parse JSON configuration ({e}) from: {config_str}"
        )
    ...
    try:
        ...
        if sp and os.path.exists(sp):
            ...
            if buf:
                ...
                _LOGGER.info(
                    f"Sent serial command '{cmd}' via dut.serial socket: {res.strip()}"
                )
                return res
            ...
            if text_buf:
                _LOGGER.info(
                    f"Sent serial command '{cmd}' via dut.serial: {text_buf.strip()}"
                )
                return text_buf
    except Exception as e:
        _LOGGER.debug(f"Serial transport is unavailable ({e})")
        return None
...
def set_usb_config(
    ...
    Args:
        dut: Fuchsia DUT device object.
        config: Function or configuration string (e.g. 'cdc,adb', 'sourcesink', 'loopback').
        reboot_if_needed: If True and restoring non-test config, triggers reboot so CDC Ethernet re-enumerates.
    """
    _LOGGER.info(
        f"Applying USB peripheral configuration '{config}' on target..."
    )

    applied = False
    ssh_res: Optional[Any] = None
    ssh_err: Optional[Exception] = None
    ...
    # 2. Fallback to SSH via dut.ffx.run_ssh_cmd if serial command did not apply it
    if not applied:
        ...
        if ffx and hasattr(ffx, "run_ssh_cmd"):
            try:
                ...
                if ssh_res and (
                    ...
                    applied = True
                    _LOGGER.info(
                        f"Applied USB peripheral configuration '{config}' via SSH"
                    )
            except Exception as e:
                ssh_err = e
                _LOGGER.debug(f"SSH execution failed: {e}")

    is_test = any(
        f in config.lower() for f in ("sourcesink", "loopback", "test")
    )

    # If test config failed to apply over serial and SSH, fail fast instead of hanging later
    if not applied and is_test:
        raise RuntimeError(
            f"Failed to apply test USB config '{config}' over serial console or SSH: "
            f"serial_res={serial_res!r}, ssh_res={ssh_res!r}, ssh_err={ssh_err!r}"
        )

    # If target was offline or restore failed, coordinate reboot to reload default functions
    if not applied and not is_test:
        _LOGGER.info(
            f"Device unresponsive during restore of '{config}'; triggering reboot to restore default functions..."
        )
        if hasattr(dut, "reboot"):
            try:
                ...
                _LOGGER.warning(f"dut.reboot() failed: {e}")
        _send_serial_command("dm reboot", timeout_sec=3.0, dut=dut)
        return

    # If restoring non-test config and reboot was requested, reboot to rebind CDC interface
