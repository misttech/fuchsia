        ...
        Ok(match buf.kind() {
            NDTA_NAME => {
                Self::Name(parse_string(payload).map_err(|error| {
                    NeighbourTableError::InvalidValue { kind: "NDTA_NAME", error }
                })?)
            }
            ...
            NDTA_PARMS => {
                Self::Parms(VecNeighbourTableParameter::parse(&NlaBuffer::new_checked(payload)?)?.0)
            }
            NDTA_GC_INTERVAL => Self::GcInterval(parse_u64(payload).map_err(|error| {
                NeighbourTableError::InvalidValue { kind: "NDTA_GC_INTERVAL", error }
            })?),
            NDTA_THRESH1 => Self::Threshold1(parse_u32(payload).map_err(|error| {
                NeighbourTableError::InvalidValue { kind: "NDTA_THRESH1", error }
            })?),
            NDTA_THRESH2 => Self::Threshold2(parse_u32(payload).map_err(|error| {
                NeighbourTableError::InvalidValue { kind: "NDTA_THRESH2", error }
            })?),
            NDTA_THRESH3 => Self::Threshold3(parse_u32(payload).map_err(|error| {
                NeighbourTableError::InvalidValue { kind: "NDTA_THRESH3", error }
            })?),
            kind => Self::Other(
