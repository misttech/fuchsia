...
impl From<&Vec<Ipv4Addr>> for BondIpAddrNlaList {
    fn from(addrs: &Vec<Ipv4Addr>) -> Self {
        ...
        for (i, addr) in addrs.iter().enumerate() {
            let nla = BondIpAddrNla { index: i as u16, addr: IpAddr::V4(*addr) };
            nlas.push(nla);
        }
...
impl From<&Vec<Ipv6Addr>> for BondIpAddrNlaList {
    fn from(addrs: &Vec<Ipv6Addr>) -> Self {
        let mut nlas = Vec::new();
        for (i, addr) in addrs.iter().enumerate() {
            let nla = BondIpAddrNla { index: i as u16, addr: IpAddr::V6(*addr) };
            nlas.push(nla);
        }
        BondIpAddrNlaList(nlas)
    }
...
impl<'a, T: AsRef<[u8]> + ?Sized> Parseable<NlaBuffer<&'a T>> for InfoBond {
    ...
    fn parse(buf: &NlaBuffer<&'a T>) -> Result<Self, DecodeError> {
        ...
        Ok(match buf.kind() {
            IFLA_BOND_MODE => {
                Self::Mode(parse_u8(payload).context("invalid IFLA_BOND_MODE value")?.into())
            }
