...
fn link_message_packet_header_read() {
    ...
    assert_eq!(
        packet.flags(),
        (LinkFlags::Loopback | LinkFlags::LowerUp | LinkFlags::Running | LinkFlags::Up).bits()
    );
...
fn link_mssage_packet_attributes_read() {
    ...
    let parsed = LinkAttribute::parse_with_param(&nla, AddressFamily::Inet).unwrap();
...
fn link_message_emit() {
    let header = LinkHeader {
        ...
        index: 1,
        flags: LinkFlags::Loopback | LinkFlags::LowerUp | LinkFlags::Running | LinkFlags::Up,
