    ...
    fn parse(buf: &NlaBuffer<&'a T>) -> Result<Self, DecodeError> {
        ...
        Ok(match buf.kind() {
            ...
            IFLA_MACSEC_CIPHER_SUITE => CipherSuite(
                parse_u64(payload).context("invalid IFLA_MACSEC_CIPHER_SUITE value")?.into(),
            ),
            IFLA_MACSEC_WINDOW => {
            ...
            IFLA_MACSEC_VALIDATION => Validation(
                parse_u8(payload).context("invalid IFLA_MACSEC_VALIDATION value")?.into(),
            ),
            IFLA_MACSEC_OFFLOAD => {
                Offload(parse_u8(payload).context("invalid IFLA_MACSEC_OFFLOAD value")?.into())
            }
