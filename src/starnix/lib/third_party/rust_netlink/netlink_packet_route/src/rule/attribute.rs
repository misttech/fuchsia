        ...
        Ok(match buf.kind() {
            FRA_DST => Self::Destination(
                parse_ip_addr(payload)
                    .map_err(|error| RuleError::InvalidValue { kind: "FRA_DST", error })?,
            ),
            FRA_SRC => Self::Source(
                parse_ip_addr(payload)
                    .map_err(|error| RuleError::InvalidValue { kind: "FRA_DST", error })?,
            ...
            FRA_IIFNAME => Self::Iifname(
                parse_string(payload)
                    .map_err(|error| RuleError::InvalidValue { kind: "FRA_IIFNAME", error })?,
            ),
            FRA_GOTO => Self::Goto(
                parse_u32(payload)
                    .map_err(|error| RuleError::InvalidValue { kind: "FRA_GOTO", error })?,
            ),
            FRA_PRIORITY => Self::Priority(
                parse_u32(payload)
                    .map_err(|error| RuleError::InvalidValue { kind: "FRA_PRIORITY", error })?,
            ),
            FRA_FWMARK => Self::FwMark(
                parse_u32(payload)
                    .map_err(|error| RuleError::InvalidValue { kind: "FRA_FWMARK", error })?,
            ),
            FRA_FLOW => Self::Realm(RouteRealm::parse(payload)?),
            FRA_TUN_ID => Self::TunId(
                parse_u32(payload)
                    .map_err(|error| RuleError::InvalidValue { kind: "FRA_TUN_ID", error })?,
            ),
            FRA_SUPPRESS_IFGROUP => Self::SuppressIfGroup(parse_u32(payload).map_err(|error| {
                RuleError::InvalidValue { kind: "FRA_SUPPRESS_IFGROUP", error }
            })?),
            FRA_SUPPRESS_PREFIXLEN => {
                Self::SuppressPrefixLen(parse_u32(payload).map_err(|error| {
                    RuleError::InvalidValue { kind: "FRA_SUPPRESS_PREFIXLEN", error }
                })?)
            }
            FRA_TABLE => Self::Table(
                parse_u32(payload)
                    .map_err(|error| RuleError::InvalidValue { kind: "FRA_TABLE", error })?,
            ),
            FRA_FWMASK => Self::FwMask(
                parse_u32(payload)
                    .map_err(|error| RuleError::InvalidValue { kind: "FRA_FWMASK", error })?,
            ),
            FRA_OIFNAME => Self::Oifname(
                parse_string(payload)
                    .map_err(|error| RuleError::InvalidValue { kind: "FRA_OIFNAME", error })?,
            ),
            FRA_L3MDEV => Self::L3MDev(
                parse_u8(payload)
                    .map_err(|error| RuleError::InvalidValue { kind: "FRA_L3MDEV", error })?
                    > 0,
            ),
            FRA_UID_RANGE => Self::UidRange(RuleUidRange::parse(payload)?),
            FRA_PROTOCOL => Self::Protocol(
                parse_u8(payload)
                    .map_err(|error| RuleError::InvalidValue { kind: "FRA_PROTOCOL", error })?
                    .into(),
            ),
            FRA_IP_PROTO => Self::IpProtocol(IpProtocol::from(
                parse_u8(payload)
                    .map_err(|error| RuleError::InvalidValue { kind: "FRA_IP_PROTO", error })?
                    as i32,
            )),
