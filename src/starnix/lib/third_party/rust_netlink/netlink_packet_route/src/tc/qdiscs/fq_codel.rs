    ...
    fn parse(buf: &NlaBuffer<&'a T>) -> Result<Self, TcError> {
        ...
        Ok(match buf.kind() {
            ...
            TCA_FQ_CODEL_LIMIT => Self::Limit(
                parse_u32(payload)
                    .map_err(|error| TcError::InvalidValue { kind: "TCA_FQ_CODEL_LIMIT", error })?,
            ),
            ...
            TCA_FQ_CODEL_ECN => Self::Ecn(
                parse_u32(payload)
                    .map_err(|error| TcError::InvalidValue { kind: "TCA_FQ_CODEL_ECN", error })?,
            ),
            TCA_FQ_CODEL_FLOWS => Self::Flows(
                parse_u32(payload)
                    .map_err(|error| TcError::InvalidValue { kind: "TCA_FQ_CODEL_FLOWS", error })?,
            ),
            ...
            TCA_FQ_CODEL_CE_THRESHOLD_SELECTOR => {
                Self::CeThresholdSelector(parse_u8(payload).map_err(|error| {
                    TcError::InvalidValue { kind: "TCA_FQ_CODEL_CE_THRESHOLD_SELECTOR", error }
                })?)
            }
