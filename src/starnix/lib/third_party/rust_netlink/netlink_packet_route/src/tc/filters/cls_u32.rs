    ...
    fn parse(buf: &NlaBuffer<&'a T>) -> Result<Self, TcError> {
        ...
        Ok(match buf.kind() {
            TCA_U32_CLASSID => Self::ClassId(TcHandle::from(
                parse_u32(payload)
                    .map_err(|error| TcError::InvalidValue { kind: "TCA_U32_UNSPEC", error })?,
            )),
            TCA_U32_HASH => Self::Hash(
                parse_u32(payload)
                    .map_err(|error| TcError::InvalidValue { kind: "TCA_U32_HASH", error })?,
            ),
            TCA_U32_LINK => Self::Link(
                parse_u32(payload)
                    .map_err(|error| TcError::InvalidValue { kind: "TCA_U32_LINK", error })?,
            ),
            ...
            TCA_U32_FLAGS => Self::Flags(TcU32OptionFlags::from_bits_retain(
                parse_u32(payload)
                    .map_err(|error| TcError::InvalidValue { kind: "TCA_U32_FLAGS", error })?,
            )),
            kind => Self::Other(
...
impl<T: AsRef<[u8]>> TcU32SelectorBuffer<T> {
    ...
    fn check_buffer_length(&self) -> Result<(), DecodeError> {
        ...
        if len < expected_len {
            return Err(
                format!("invalid RouteNextHopBuffer: length {} < {}", len, expected_len,).into()
            );
        }
        Ok(())
    }
...
impl<T: AsRef<[u8]>> Parseable<TcU32KeyBuffer<T>> for TcU32Key {
    ...
    fn parse(buf: &TcU32KeyBuffer<T>) -> Result<Self, ()> {
        Ok(Self { mask: buf.mask(), val: buf.val(), off: buf.off(), offmask: buf.offmask() })
    }
}
