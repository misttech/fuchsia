        ...
        Ok(match buf.kind() {
            TCA_KIND => TcAttribute::Kind(
                parse_string(payload)
                    .map_err(|error| TcError::InvalidValue { kind: "TCA_KIND", error })?,
            ),
            ...
            TCA_CHAIN => TcAttribute::Chain(
                parse_u32(payload)
                    .map_err(|error| TcError::InvalidValue { kind: "TCA_CHAIN", error })?,
            ),
            TCA_HW_OFFLOAD => TcAttribute::HwOffload(
                parse_u8(payload)
                    .map_err(|error| TcError::InvalidValue { kind: "TCA_HW_OFFLOAD", error })?,
            ),
