...
fn main() {
    ...
    let mut genlmsg =
        GenlMessage::from_payload(GenlCtrl { cmd: GenlCtrlCmd::GetFamily, nlas: vec![] });
    genlmsg.finalize();
...
fn print_entry(entry: Vec<GenlCtrlAttrs>) {
    ...
    let version = entry
        .iter()
        .find_map(|nla| if let GenlCtrlAttrs::Version(ver) = nla { Some(*ver) } else { None })
        .expect("Cannot find Version attribute");
    let hdrsize = entry
        .iter()
        .find_map(|nla| if let GenlCtrlAttrs::HdrSize(hdr) = nla { Some(*hdr) } else { None })
