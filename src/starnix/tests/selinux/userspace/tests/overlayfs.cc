...
class OverlayFsTest : public ::testing::Test {
 protected:
  ...
  void TearDown() override { ASSERT_THAT(umount(overlay_.c_str()), SyscallSucceeds()); }
...
TEST_F(OverlayFsTest, ClearAppendNotDeniedIfMounterLacksWrite) {
  ...
  EXPECT_TRUE(RunSubprocessAs("test_u:test_r:test_overlayfs_caller_t:s0", [&] {
    int fd = open((overlay_ + "/file").c_str(), O_WRONLY | O_APPEND);
    ...
    EXPECT_THAT(fcntl(fd, F_SETFL, flags & ~O_APPEND), SyscallSucceeds());

    EXPECT_THAT(lseek(fd, 0, SEEK_SET), SyscallSucceeds());
    EXPECT_THAT(write(fd, "X", 1), SyscallSucceeds());
  }));
}
...
TEST_F(OverlayFsTest, CopyUpDeniedIfMounterLacksWrite) {
  ...
  EXPECT_TRUE(RunSubprocessAs("test_u:test_r:test_overlayfs_caller_t:s0", [&] {
    int fd = open((overlay_ + "/file").c_str(), O_WRONLY);
    EXPECT_THAT(fd, SyscallFailsWithErrno(EACCES));
  }));
}
...
TEST_F(OverlayFsTest, AuditChecksFileOpenAndWrite) {
  ...
  ASSERT_TRUE(RunSubprocessAs("test_u:test_r:test_overlayfs_caller_t:s0", [&] {
    int fd = open((overlay_ + "/file").c_str(), O_WRONLY);
    ASSERT_THAT(fd, SyscallSucceeds());
    ASSERT_THAT(write(fd, "test", 4), SyscallSucceeds());
  }));
}
...
TEST_F(OverlayFsTest, AuditChecksFileOpenAndRead) {
  ...
  ASSERT_TRUE(RunSubprocessAs("test_u:test_r:test_overlayfs_caller_t:s0", [&] {
    int fd = open((overlay_ + "/file").c_str(), O_RDONLY);
    ASSERT_THAT(fd, SyscallSucceeds());
    char buf[1];
    ASSERT_THAT(read(fd, &buf, sizeof(buf)), SyscallSucceeds());
  }));
}
