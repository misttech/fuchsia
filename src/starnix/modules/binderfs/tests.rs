...
pub mod tests {
    ...
    use crate::thread::{
        BinderThread, Command, CommandQueueWithWaitQueue, IndexedCommandQueue, QueuedCommand,
        RegistrationState, RequeueEventRegistration, TransactionError, TransactionRole,
        TransactionSender, WeakBinderPeer,
    };
    ...
    impl ResourceAccessor for AutoReleasableTask {
        ...
        fn add_files_with_flags(
            ...
            self.deref().add_files_with_flags(current_task, files, add_action)
        }
    ...
    async fn handle_is_not_dropped_after_transaction_finishes_if_it_already_existed() {
        spawn_kernel_and_run(async |current_task| {
            let device = BinderDevice::default();
            let proc_1 = BinderProcessFixture::new(current_task, &device);
            let proc_2 = BinderProcessFixture::new(current_task, &device);
    ...
    async fn binder_object_enqueues_release_command_when_dropped() {
        spawn_kernel_and_run(async |current_task| {
            ...
            assert_matches!(
                ...
                Some(QueuedCommand { command: Command::ReleaseRef(LOCAL_BINDER_OBJECT), .. })
            );
        })
        .await;
    }
    ...
    async fn copy_transaction_data_between_processes() {
        spawn_kernel_and_run(async |current_task| {
            ...
            *receiver.proc.shared_memory.lock() = Some(
            ...
            let data_addr = map_memory(&current_task, UserAddress::default(), *PAGE_SIZE);
    ...
    async fn transaction_translate_binder_leaving_process() {
        spawn_kernel_and_run(async |current_task| {
            ...
            assert_matches!(
                ...
                Some(QueuedCommand { command: Command::AcquireRef(BINDER_OBJECT), .. })
            );
            transaction_state.release(());
        })
        .await;
    ...
    async fn transaction_translate_binder_handles_with_same_address() {
        spawn_kernel_and_run(async |current_task| {
            ...
            let sender = BinderProcessFixture::new_current(current_task, &device);
            let receiver = BinderProcessFixture::new(current_task, &device);
            let other_proc = BinderProcessFixture::new(current_task, &device);
            let mut receiver_shared_memory = receiver.lock_shared_memory();
            let mut allocations =
                receiver_shared_memory.allocate_buffers(0, 0, 0, 0).expect("allocate buffers");
    ...
    async fn transaction_translate_buffers() {
        spawn_kernel_and_run(async |current_task| {
            ...
            // binder driver.
            let sender_addr = map_memory(&current_task, UserAddress::default(), *PAGE_SIZE);
    ...
    async fn transaction_translate_fd_array() {
        spawn_kernel_and_run(async |current_task| {
            ...
            current_task.add_file(file, FdFlags::empty()).unwrap();

            // Open two files in the sender process. These will be sent in the transaction.
            let file1 = PanickingFile::new_file(&current_task);
            let file2 = PanickingFile::new_file(&current_task);
            ...
            // Perform the translation and copying.
            device
                .handle_transaction(&sender.context(current_task), &mut Vec::new(), input)
    ...
    async fn transaction_fd_array_sender_cancels() {
        spawn_kernel_and_run(async |current_task| {
            ...
            // sender, potentially hiding a bug.
            let file1 = PanickingFile::new_file(&current_task);
            current_task.add_file(file1, FdFlags::empty()).unwrap();
    ...
    fn open_binder_fd(current_task: &CurrentTask, binder_driver: &BinderDevice) -> FileHandle {
        ...
        let binder = binder_driver
            .open(&current_task, DeviceId::NONE, &node, OpenFlags::RDWR)
            .expect("binder dev open failed");
        FileObject::new_anonymous(current_task, binder, node.entry.node.clone(), OpenFlags::RDWR)
    }
    ...
    async fn death_notification_fires_when_process_dies() {
        spawn_kernel_and_run(async |current_task| {
            ...
            assert_matches!(
                ...
                Some(QueuedCommand { command: Command::DeadBinder(DEATH_NOTIFICATION_COOKIE), .. })
            );
        })
        .await;
    }
    ...
    async fn death_notification_is_cleared_before_process_dies() {
        spawn_kernel_and_run(async |current_task| {
            ...
            // Check that the client received an acknowlgement
            {
                ...
                assert_matches!(
                    queue.front().unwrap(),
                    QueuedCommand { command: Command::ClearDeathNotificationDone(_), .. }
                );
    ...
    async fn send_fd_in_transaction_with_prefetched_files() {
        spawn_kernel_and_run(async |current_task| {
            ...
            // Open a file in the sender process.
            let file = new_null_file(&current_task, OpenFlags::empty());
            let sender_fd =
                current_task.add_file(file.clone(), FdFlags::empty()).expect("add file");
    ...
    async fn cleanup_fd_in_failed_transaction() {
        spawn_kernel_and_run(async |current_task| {
            ...
            // Open a file in the sender process.
            let file = PanickingFile::new_file(&current_task);
            let sender_fd = current_task.add_file(file, FdFlags::CLOEXEC).expect("add file");
    ...
    async fn cleanup_refs_in_successful_transaction() {
        spawn_kernel_and_run(async |current_task| {
            ...
            assert_matches!(
                ...
                Some(QueuedCommand { command: Command::ReleaseRef(BINDER_OBJECT), .. })
            );
        })
        .await;
    }
    ...
    async fn transaction_error_dispatch() {
        spawn_kernel_and_run(async |current_task| {
            ...
            TransactionError::Malformed(errno!(EINVAL))
                .dispatch(&proc.thread)
                .expect("no error");
            assert_matches!(
                proc.thread.lock().command_queue.pop_front(),
                Some(QueuedCommand { command: Command::Error(val), .. }) if val == EINVAL.return_value() as i32
            );
    ...
    async fn dead_reply_when_transaction_recipient_proc_dies() {
        spawn_kernel_and_run(async |current_task| {
            ...
            assert_matches!(
                receiver.proc.lock().command_queue.front(),
                Some(QueuedCommand { command: Command::Transaction { .. }, .. })
            );
    ...
    async fn dead_reply_when_transaction_recipient_proc_dies_not_top_transaction() {
        spawn_kernel_and_run(async |current_task| {
            ...
            let sender = BinderProcessFixture::new_current(current_task, &device);
            let receiver = BinderProcessFixture::new(current_task, &device);
            let second_receiver = BinderProcessFixture::new(current_task, &device);
    ...
    async fn test_process_queue_unhandled_command_stuck_during_synchronous_call() {
        spawn_kernel_and_run(async |current_task| {
            ...
            proc_a.proc.enqueue_command(oneway_to_a.into());
    ...
    async fn connect_to_multiple_binder() {
        spawn_kernel_and_run(async |current_task| {
            ...
            let _d1 = open_binder_fd(&current_task, &driver);
            let _d2 = open_binder_fd(&current_task, &driver);
        })
        .await;
    }
    ...
    async fn remote_binder_task() {
        ...
        spawn_kernel_and_run(async |task| {
            ...
            let fds = remote_binder_task
                .add_files_with_flags(&task, files, &mut |_| {})
    ...
    async fn freeze_notification_fires_when_process_frozen() {
        spawn_kernel_and_run(async |current_task| {
            ...
            assert_matches!(
                ...
                Some(QueuedCommand {
                    command: Command::FrozenBinder(binder_frozen_state_info { is_frozen: 0, .. }),
                    ..
                })
            );
            ...
            assert_matches!(
                ...
                Some(QueuedCommand {
                    command: Command::FrozenBinder(binder_frozen_state_info { is_frozen: 1, .. }),
                    ..
                })
            );
        })
        ...
        spawn_kernel_and_run(async |current_task| {
            ...
            // Check that the pending FrozenBinder was cancelled, leaving only ClearFreezeNotificationDone.
            {
                ...
                assert!(matches!(
                    queue.pop_front(),
                    Some(QueuedCommand {
                        command: Command::ClearFreezeNotificationDone(FREEZE_NOTIFICATION_COOKIE),
                        ..
                    })
                ));
            }
    ...
    async fn refcount_command_queue_coalescing() {
        spawn_kernel_and_run(async |_current_task| {
            ...
            queue.push_back(Command::AcquireRef(local_1).into());
            assert_eq!(queue.commands.len(), 1);

            queue.push_back(Command::ReleaseRef(local_1).into());
            // Both should be coalesced / cancelled out!
            assert_eq!(queue.commands.len(), 0);

            // 2. Push IncRef for local_2, then DecRef for local_2.
            queue.push_back(Command::IncRef(local_2).into());
            assert_eq!(queue.commands.len(), 1);

            queue.push_back(Command::DecRef(local_2).into());
            // Both should be coalesced / cancelled out!
            assert_eq!(queue.commands.len(), 0);

            // 3. Different addresses should not coalesce each other.
            queue.push_back(Command::AcquireRef(local_1).into());
            queue.push_back(Command::ReleaseRef(local_2).into());
    ...
    async fn transient_refcount_cancellation_in_waiting_ack() {
        spawn_kernel_and_run(async |current_task| {
            ...
            owner.thread.lock().command_queue.push_back(Command::ReleaseRef(local_obj).into());
    ...
    async fn indexed_command_queue_operations() {
        spawn_kernel_and_run(async |_current_task| {
            ...
            assert!(queue.push_back(Command::AcquireRef(obj_1).into()));
            assert!(queue.push_back(Command::IncRef(obj_2).into()));
            assert!(queue.push_back(Command::AcquireRef(obj_3).into()));
            ...
            assert!(!queue.push_back(Command::ReleaseRef(obj_1).into()));
            assert_eq!(queue.len(), 1);
            ...
            assert_matches!(
                queue.front(),
                Some(QueuedCommand { command: Command::AcquireRef(o), .. }) if *o == obj_3
            );
            let popped = queue.pop_front().unwrap();
            assert_matches!(popped.command, Command::AcquireRef(o) if o == obj_3);
    ...
    async fn indexed_command_queue_distinct_objects_and_interleaved() {
        spawn_kernel_and_run(async |_current_task| {
            ...
            assert!(queue.push_back(Command::AcquireRef(obj_a).into()));
            assert!(queue.push_back(Command::ReleaseRef(obj_b).into()));
            assert_eq!(queue.len(), 2);
            ...
            assert!(queue.push_back(Command::AcquireRef(obj_a).into()));
            assert!(queue.push_back(Command::TransactionComplete.into()));
            // Pushing ReleaseRef(obj_a) coalesces with AcquireRef(obj_a) across TransactionComplete.
            assert!(!queue.push_back(Command::ReleaseRef(obj_a).into()));
            assert_eq!(queue.len(), 1);
            assert_matches!(queue.pop_front().unwrap().command, Command::TransactionComplete);
            assert!(queue.is_empty());
        })
        .await;
    }
    ...
    async fn indexed_command_queue_retain() {
        spawn_kernel_and_run(async |_current_task| {
            ...
            queue.push_back(Command::AcquireRef(obj_1).into());
            queue.push_back(Command::FrozenBinder(freeze_info_1).into());
            queue.push_back(Command::IncRef(obj_2).into());
            queue.push_back(Command::FrozenBinder(freeze_info_2).into());
            assert_eq!(queue.len(), 4);

            // Retain only commands that are not FrozenBinder with cookie 0x1111.
            queue.retain(
                |queued| !matches!(queued.command, Command::FrozenBinder(info) if info.cookie == 0x1111),
            );
            assert_eq!(queue.len(), 3);
            ...
            assert_matches!(queue.pop_front().unwrap().command, Command::IncRef(o) if o == obj_2);
            assert_matches!(
                queue.pop_front().unwrap().command,
    ...
    async fn multi_command_batch_read() {
        spawn_kernel_and_run(async |current_task| {
            ...
            // Enqueue 3 commands: AcquireRef (20 bytes), IncRef (20 bytes), TransactionComplete (4 bytes)
            ...
            // All 3 commands should be read in a single handle_thread_read call!
            assert_eq!(bytes_read, 20 + 20 + 4);
    ...
    #[fuchsia::test]
    async fn transaction_complete_is_terminal_before_reply() {
        spawn_kernel_and_run(async |current_task| {
            let device = BinderDevice::default();
            ...
            let trace_id = fuchsia_trace::Id::new();
            {
                ...
                thread_a.transactions.push(TransactionRole::Sender(TransactionSender {
                    ...
                    target_thread: Some(proc_b.thread.tid),
                    is_alive: true,
                    target_thread_handle: Some(proc_b.thread.thread.clone()),
                    trace_id,
                }));
                // Enqueue TransactionComplete in proc_a's queue
                thread_a
                    .enqueue_command(QueuedCommand::new(Command::TransactionComplete, trace_id));
            }

            // Setup receiver transaction on proc_b and reply
            {
            ...
            // First read returns only TransactionComplete (4 bytes) because it is a terminal command.
            let read_buffer_addr = map_memory(current_task, UserAddress::default(), *PAGE_SIZE);
            let bytes_read = device
                .handle_thread_read(
                    &proc_a.context(current_task),
                ...
                .expect("proc_a handle_thread_read");

            assert_eq!(bytes_read, 4);

            // Second read returns the Reply (68 bytes: 4 byte code + 64 byte binder_transaction_data).
            let bytes_read = device
                .handle_thread_read(
                    &proc_a.context(current_task),
                    &UserBuffer { address: read_buffer_addr, length: *PAGE_SIZE as usize },
    ...
    #[fuchsia::test]
    async fn multi_command_read_dead_reply_peeking() {
        spawn_kernel_and_run(async |current_task| {
            let device = BinderDevice::default();
            let proc = BinderProcessFixture::new_current(current_task, &device);
            let trace_id = fuchsia_trace::Id::new();

            // Push a dead sender transaction directly onto the thread's transaction stack.
            proc.thread.lock().transactions.push(TransactionRole::Sender(TransactionSender {
                target_proc: 9999,
                target_thread: None,
                is_alive: false,
                target_thread_handle: None,
                trace_id,
            }));

            let read_buffer_addr = map_memory(current_task, UserAddress::default(), *PAGE_SIZE);

            // Reading with insufficient buffer (< 4 bytes) returns ENOMEM and preserves the dead transaction.
            let err = device
                .handle_thread_read(
                    &proc.context(current_task),
                    &UserBuffer { address: read_buffer_addr, length: 2 },
                )
                .expect_err("should fail with ENOMEM");
            assert_eq!(err, errno!(ENOMEM));
            assert_eq!(proc.thread.lock().transactions.len(), 1);

            // Reading with sufficient buffer succeeds and returns BR_DEAD_REPLY (4 bytes).
            let bytes_read = device
                .handle_thread_read(
                    &proc.context(current_task),
                ...
                .expect("handle_thread_read");
            assert_eq!(bytes_read, 4);
            assert!(proc.thread.lock().transactions.is_empty());
        })
        .await;
    }

    #[fuchsia::test]
    async fn handle_thread_read_spawns_looper_when_requested() {
        spawn_kernel_and_run(async |current_task| {
            let device = BinderDevice::default();
            let proc = BinderProcessFixture::new_current(current_task, &device);

            // Configure process to allow auxiliary threads and register current thread as Main looper.
            {
                let mut proc_state = proc.proc.lock();
                proc_state.max_thread_count = 5;
                proc.thread.lock().registration = RegistrationState::Main;
            }

            // `should_request_thread` is now true.
            let read_buffer_addr = map_memory(current_task, UserAddress::default(), *PAGE_SIZE);

            // 1. If user buffer is smaller than 4 bytes, fails with ENOMEM and does not mark thread_requested.
            let err = device
                .handle_thread_read(
                    &proc.context(current_task),
                    &UserBuffer { address: read_buffer_addr, length: 2 },
                ...
                .expect_err("should fail with ENOMEM");
            assert_eq!(err, errno!(ENOMEM));
            assert!(!proc.proc.lock().thread_requested);

            // 2. Reading with sufficient buffer returns BR_SPAWN_LOOPER (4 bytes) standalone and marks thread_requested.
            let bytes_read = device
                .handle_thread_read(
                    &proc.context(current_task),
                    &UserBuffer { address: read_buffer_addr, length: *PAGE_SIZE as usize },
