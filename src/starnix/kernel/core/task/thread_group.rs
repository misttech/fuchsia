...
impl ThreadGroup {
    ...
    fn maybe_notify_tracer(
        ...
        let mut state = self.write();
        if state.zombie_ptracees.has_tracee(tracee.tid.id) {
            if self == parent {
                ...
                if let Some(zombie_notification) = state.zombie_ptracees.detach(pids, tracee.tid.id)
                {
                ...
                // continuously.
                let mut state = self.write();
                state.zombie_ptracees.set_parent_of(tracee.tid.id, Some(zombie), parent);
