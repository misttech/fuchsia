...
pub(in crate::core__::security) fn create_test_file(
...
pub(in crate::core__::security) fn create_directory_with_parents(
    dir_names: Vec<&FsStr>,
    locked: &mut Locked<Unlocked>,
    current_task: &CurrentTask,
) -> NamespaceNode {
...
pub(in crate::core__::security) fn spawn_kernel_with_selinux_hooks_test_policy_and_run<F>(
    callback: F,
) -> impl Future<Output = ()>
where
...
pub(in crate::core__::security) fn create_test_executable(
    locked: &mut Locked<Unlocked>,
    current_task: &CurrentTask,
    security_context: &[u8],
) -> NamespaceNode {
