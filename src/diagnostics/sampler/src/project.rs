    ...
    fn convert_to_metric<'b>(
        ...
        match metric.metric_type {
            ...
            MetricType::IntHistogram => {
                ...
                let cached_value = match self.cache.borrow_mut().entry(metric.metric_id) {
                    // no need to validate diff between the cached value and the current
                    // value, because
                    // 1) We know we're on the correct poll interval due to prior validation
                    // 2) There is no alternate way of using a histogram that does not use
                    //    diffing behavior
                    Entry::Occupied(mut v) => Some(v.insert(prop[0].clone().into_owned())),
                    Entry::Vacant(v) => {
                        v.insert(prop[0].clone().into_owned());
                        None
                    }
                ...
                let Some(payload) = Self::process_int_histogram(&prop[0], cached_value.as_ref())?
                else {
                    return Ok(None);
                };
...
mod test {
    ...
    use fidl_fuchsia_metrics::{HistogramBucket, MetricEventLoggerMarker, MetricEventPayload};
    ...
    fn compute_histogram_diff_decreasing_count_is_error() {
        ...
        assert_matches!(result, Error::HistogramBucketCountDecreased);
    }
