...
// TODO(https://fxbug.dev/559081732): Remove V1 InputReportsReader implementation once
// print-input-report supports InputReportsReaderV2.
...
/// Represents a [`fuchsia.input.report.InputReportsReader/ReadInputReports`]
/// response.
pub struct ReadInputReportsResponse {
    responder: fidl_next::Responder<fidl_input_report::input_reports_reader::ReadInputReports>,
    reports: Vec<fidl_input_report::InputReport>,
}

impl ReadInputReportsResponse {
    pub async fn send(self) {
        let res = self.responder.respond(self.reports).await;
        if let Err(e) = res {
            log::error!("Failed to send reports from FIFO: {:?}", e);
        }
    }
}

/// Couples incoming hardware touch reports to waiting `ReadInputReport`
/// responders.
///
/// Buffers input reports and sends them to clients when they request data,
/// implementing the "Hanging Get" pattern for the
/// [`fuchsia.input.report.InputReportsReader`] protocol.
pub struct InputReportReaderCoupler {
    pending_reports: VecDeque<fidl_input_report::InputReport>,
    pending_responder:
        Option<fidl_next::Responder<fidl_input_report::input_reports_reader::ReadInputReports>>,
}

impl InputReportReaderCoupler {
    pub fn new() -> Self {
        Self {
    ...
    /// Adds a report to the FIFO queue.
    /// If the queue is full, the oldest report is dropped.
    pub fn push_report(&mut self, report: fidl_input_report::InputReport) {
        if self.pending_reports.len() >= MAX_DEVICE_REPORT_COUNT {
            log::debug!(
                "InputReportReaderCoupler pending report queue full; dropping oldest report"
            );
    ...
    /// Tries to set `responder` for a pending read request.
    ///
    /// Returns an Err with the given `responder` if there is already a pending
    /// responder.
    pub fn try_set_responder(
        &mut self,
        responder: fidl_next::Responder<fidl_input_report::input_reports_reader::ReadInputReports>,
    ) -> Result<(), fidl_next::Responder<fidl_input_report::input_reports_reader::ReadInputReports>>
    {
        if self.pending_responder.is_some() {
            Err(responder)
        } else {
    ...
    /// Returns the response action that is ready to be performed (i.e. there is
    /// a pending responder and at least one pending report).
    ///
    /// Otherwise returns `None`.
    pub fn take_pending_response(&mut self) -> Option<ReadInputReportsResponse> {
        if self.pending_reports.is_empty() {
            return None;
        }
        let responder = self.pending_responder.take()?;
        let reports = self.pending_reports.drain(..).collect();
        Some(ReadInputReportsResponse { responder, reports })
    }
}

#[derive(Clone)]
pub struct SharedInputReportReaderCoupler(Arc<Mutex<InputReportReaderCoupler>>);

impl SharedInputReportReaderCoupler {
    pub fn new() -> Self {
        Self(Arc::new(Mutex::new(InputReportReaderCoupler::new())))
    }
}

impl Deref for SharedInputReportReaderCoupler {
    type Target = Mutex<InputReportReaderCoupler>;
...
/// Handles [`fuchsia.input.report.InputReportsReader`] protocol requests.
pub struct HangingGetReaderFidlHandler {
    reader_coupler: SharedInputReportReaderCoupler,
}

impl HangingGetReaderFidlHandler {
    pub fn new(reader_coupler: SharedInputReportReaderCoupler) -> Self {
        Self { reader_coupler }
    }
}

impl fidl_input_report::InputReportsReaderServerHandler for HangingGetReaderFidlHandler {
    async fn read_input_reports(
        &mut self,
        responder: fidl_next::Responder<fidl_input_report::input_reports_reader::ReadInputReports>,
    ) {
        let take_response_result = {
            let mut reader_coupler = self.reader_coupler.lock();
            match reader_coupler.try_set_responder(responder) {
                Ok(()) => Ok(reader_coupler.take_pending_response()),
                Err(responder) => Err(responder),
            }
        };

        match take_response_result {
            Ok(Some(response)) => {
                response.send().await;
            }
            Ok(None) => {
                log::debug!("No pending reports to send.");
            }
            Err(responder) => {
                log::error!("Reader is already bound to a pending read request.");
                let _ = responder.respond_err(zx::Status::ALREADY_BOUND).await;
            }
        }
    }
}

/// Binds together a hanging-get reader with the background task that cleans it up
/// when the FIDL channel closes.
pub struct HangingGetReaderEntry {
    id: usize,
    reader_coupler: SharedInputReportReaderCoupler,
    #[expect(unused)]
    handler_and_cleanup_task: fuchsia_async::Task<()>,
}

impl HangingGetReaderEntry {
    /// Creates a new `HangingGetReaderEntry` and spawns the FIDL server loop.
    ///
    /// When the FIDL channel closes, `on_disconnect` is invoked to perform cleanup.
    pub fn new(
        id: usize,
        server_end: fidl_next::ServerEnd<fidl_input_report::InputReportsReader>,
        on_disconnect: impl FnOnce() + Send + 'static,
    ) -> Self {
        let reader_coupler = SharedInputReportReaderCoupler::new();
        let handler = HangingGetReaderFidlHandler::new(reader_coupler.clone());
        let handler_task = server_end.spawn(handler);
        let handler_and_cleanup_task = fuchsia_async::Task::spawn(async move {
            let _ = handler_task.await;
            on_disconnect();
        });
        Self { id, reader_coupler, handler_and_cleanup_task }
    }
    ...
    /// Pushes an input report to the reader FIFO and retrieves any ready response.
    pub fn push_report(
        &self,
        report: fidl_input_report::InputReport,
    ) -> Option<ReadInputReportsResponse> {
        let mut coupler = self.reader_coupler.lock();
        coupler.push_report(report);
        coupler.take_pending_response()
    }
}
...
mod tests {
    ...
    async fn test_reader_reports() {
        ...
        #[expect(unused)]
        let handler_task = reader_server_end.spawn(handler);
        let reader_client = reader_client_end.spawn();

        {
            let mut coupler = reader_coupler.lock();
            coupler.push_report(make_test_input_report(100, 200));
        }

        let res = reader_client.read_input_reports().await.unwrap().unwrap();
        let reports = res.reports;
        assert_eq!(reports.len(), 1);
        let contacts = reports[0].touch.as_ref().unwrap().contacts.as_ref().unwrap();
        assert_eq!(contacts[0].position_x, Some(100));
        assert_eq!(contacts[0].position_y, Some(200));
    }

    #[fuchsia::test]
    async fn test_reader_hanging_get() {
        let reader_coupler = SharedInputReportReaderCoupler::new();
        let handler = HangingGetReaderFidlHandler::new(reader_coupler.clone());
        ...
        // 1. Issue read_input_reports in a background task while FIFO is empty (hanging get)
        let read_task =
            fasync::Task::spawn(async move { reader_client.read_input_reports().await });

        // Wait until server handler processes read_input_reports and sets pending responder
        while reader_coupler.lock().pending_responder.is_none() {
            fasync::yield_now().await;
        }

        // 2. Push report to coupler and take pending response to send
        let response = {
            let mut coupler = reader_coupler.lock();
            coupler.push_report(make_test_input_report(100, 200));
            coupler.take_pending_response()
        ...
        // 3. Ensure the pending hanging get receives the report
        let res = read_task.await.unwrap().unwrap();
        assert_eq!(res.reports.len(), 1);
        let contacts = res.reports[0].touch.as_ref().unwrap().contacts.as_ref().unwrap();
        assert_eq!(contacts[0].position_x, Some(100));
        assert_eq!(contacts[0].position_y, Some(200));
    }
