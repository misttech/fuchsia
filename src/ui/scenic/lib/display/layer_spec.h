...
#ifndef SRC_UI_SCENIC_LIB_DISPLAY_LAYER_SPEC_H_
#define SRC_UI_SCENIC_LIB_DISPLAY_LAYER_SPEC_H_
...
// Variant sub-type of `LayerSpec`.
//
// Represents the subset of an Image Layer's configuration that is relevant for the
// `fuchsia.hardware.display.Coordinator/CheckConfig()` method. This includes properties like
...
struct ImageLayerSpec {
  ...
  constexpr bool operator==(const ImageLayerSpec& other) const {
...
// Variant sub-type of `LayerSpec`.
//
// Represents the subset of a Color Layer's configuration that is relevant for
// the `fuchsia.hardware.display.Coordinator/CheckConfig()` method.
struct ColorLayerSpec {
  WireColor color = {
  ...
  constexpr bool operator==(const ColorLayerSpec& other) const {
    return color.format == other.color.format && color.bytes == other.color.bytes &&
           display_destination == other.display_destination;
  }
};

// Variant sub-type of `LayerSpec`.  This is the default value for a new `LayerSpec`.
struct UninitializedLayerSpec {
  constexpr bool operator==(const UninitializedLayerSpec& other) const { return true; }
};

// Represents the `CheckConfig()`-relevant properties of a single layer.  This is a variant type
// which initially holds an `InvalidLayerSpec`, and which can subsequently be set to either an
// `ImageLayerSpec` or a `ColorLayerSpec`.
struct LayerSpec {
  std::variant<UninitializedLayerSpec, ImageLayerSpec, ColorLayerSpec> config;

  constexpr bool operator==(const LayerSpec& other) const { return config == other.config; }

  LayerSpec& operator=(const ImageLayerSpec& image_ls) {
    config = image_ls;
    return *this;
  }

  LayerSpec& operator=(const ColorLayerSpec& color_ls) {
    config = color_ls;
    return *this;
  }
};
...
template <>
struct hash<display::internal::ImageLayerSpec> {
  std::size_t operator()(const display::internal::ImageLayerSpec& image_ls) {
...
template <>
struct hash<display::internal::ColorLayerSpec> {
  std::size_t operator()(const display::internal::ColorLayerSpec& color_ls) {
    // Random seed (`openssl rand -hex 8`) avoids collisions with types with the same memory layout.
...
template <>
struct hash<display::internal::UninitializedLayerSpec> {
  std::size_t operator()(const display::internal::UninitializedLayerSpec& uninitialized_ls) {
    // Random seed (`openssl rand -hex 8`) avoids collisions with types with the same memory layout.
...
template <>
struct hash<display::internal::LayerSpec> {
  std::size_t operator()(const display::internal::LayerSpec& ls) {
    // Random seed (`openssl rand -hex 8`) avoids collisions with types with the same memory layout.
    std::size_t seed = 0x06cc66fb9d5ca0c5;
    if (const display::internal::ImageLayerSpec* image_ls =
            std::get_if<display::internal::ImageLayerSpec>(&ls.config)) {
      types::hash_combine(seed, *image_ls);
      return seed;
    }
    if (const display::internal::ColorLayerSpec* color_ls =
            std::get_if<display::internal::ColorLayerSpec>(&ls.config)) {
      types::hash_combine(seed, *color_ls);
      return seed;
    }
    if (const display::internal::UninitializedLayerSpec* uninitialized_ls =
            std::get_if<display::internal::UninitializedLayerSpec>(&ls.config)) {
...
#endif  // SRC_UI_SCENIC_LIB_DISPLAY_LAYER_SPEC_H_
