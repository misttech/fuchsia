...
mod test {
    ...
    async fn drag_keep_button_down() {
        ...
        let inputs = vec![
            touchpad_event(vec![finger_pos0_um], SortedVecSet::from(vec![1])),
            touchpad_event(vec![finger_pos1_um], SortedVecSet::from(vec![1])),
            touchpad_event(vec![finger_pos2_um], SortedVecSet::from(vec![1])),
        ];
    ...
    async fn drag_keep_contact() {
        ...
        let inputs = vec![
            touchpad_event(vec![finger_pos0_um], SortedVecSet::from(vec![1])),
            touchpad_event(vec![finger_pos1_um], SortedVecSet::from(vec![1])),
            touchpad_event(vec![finger_pos2_um], SortedVecSet::new()),
        ];
        let got = utils::run_gesture_arena_test(inputs).await;
    ...
    async fn place_finger_button_down_drag_release_button_lift() {
        ...
        let inputs = vec![
            ...
            touchpad_event(vec![finger_pos0_um], SortedVecSet::new()),
            // Down -> claimed by OneFingerButton because button down.
            touchpad_event(vec![finger_pos1_um], SortedVecSet::from(vec![1])),
            // Move -> claimed by OneFingerButton.
            touchpad_event(vec![finger_pos2_um], SortedVecSet::from(vec![1])),
            // Hold.
            touchpad_event(vec![finger_pos3_um], SortedVecSet::from(vec![1])),
            // Release.
            touchpad_event(vec![], SortedVecSet::new()),
        ];
        let got = utils::run_gesture_arena_test(inputs).await;
    ...
    mod chain {
        ...
        async fn drag_release_button_then_move() {
            ...
            let inputs = vec![
                ...
                touchpad_event(vec![finger_pos3_um], SortedVecSet::new()),
                touchpad_event(vec![finger_pos4_um], SortedVecSet::new()),
            ];
            let got = utils::run_gesture_arena_test(inputs).await;
