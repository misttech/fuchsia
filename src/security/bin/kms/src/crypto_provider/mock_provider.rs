...
impl CryptoProvider for MockProvider {
    ...
    fn generate_asymmetric_key(
        ...
        *self.key_name.lock() = Some(key_name.to_string());
        let result: &Result<Vec<u8>, CryptoProviderError> = &self.result.lock();
        match result {
    ...
    fn import_asymmetric_key(
        ...
        *self.key_name.lock() = Some(key_name.to_string());
        *self.key_data.lock() = Some(key_data.to_vec());
        let result: &Result<Vec<u8>, CryptoProviderError> = &self.result.lock();
        match result {
            Err(err) => Err(err.clone()),
...
impl MockProvider {
    ...
    pub fn set_key_result(&self, key_result: Result<Vec<u8>, CryptoProviderError>) {
        *self.key_result.lock() = key_result;
    }
    ...
    pub fn get_called_key_data(&self) -> Vec<u8> {
        let key_data: Option<Vec<u8>> = self.key_data.lock().clone();
        key_data.unwrap()
    }
    ...
    pub fn get_called_key_name(&self) -> String {
        let key_name: Option<String> = self.key_name.lock().clone();
        key_name.unwrap()
    }
}
