...
zx_status_t guest_ethernet_create(GuestEthernetContext* context,
  ...
  auto guest_ethernet = std::make_unique<GuestEthernet>(
      context->SyncDispatcher(),
      network::DeviceInterfaceDispatchers{context->ImplDispatcher(), context->IfcDispatcher(),
                                          context->PortDispatcher()},
      network::ShimDispatchers{context->ShimDispatcher(), context->ShimPortDispatcher()});
