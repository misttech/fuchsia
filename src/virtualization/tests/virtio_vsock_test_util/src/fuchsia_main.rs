...
async fn main() -> Result<(), Error> {
    ...
    vsock.listen(8001, acceptor_client).await?.map_err(zx::Status::err_from_raw)?;
    let (acceptor_client2, acceptor2) = endpoints::create_endpoints::<AcceptorMarker>();
    vsock.listen(8002, acceptor_client2).await?.map_err(zx::Status::err_from_raw)?;
    let (acceptor_client3, acceptor3) = endpoints::create_endpoints::<AcceptorMarker>();
    vsock.listen(8003, acceptor_client3).await?.map_err(zx::Status::err_from_raw)?;
    ...
    let _port = vsock.connect(2, 8000, con).await?.map_err(zx::Status::err_from_raw)?;
