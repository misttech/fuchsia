...
class UnitSymbolFactory {
 public:
  ...
  UnitSymbolFactory(fxl::RefPtr<const SymbolFactory> factory, DwarfDieRef unit_ref)
      : symbol_factory_(std::move(factory)), unit_ref_(unit_ref) {}
  ...
  LazySymbol MakeLazyUnitRelative(uint64_t offset_from_unit) const {
    if (symbol_factory_ && unit_ref_.is_valid())
      return LazySymbol(symbol_factory_, unit_ref_ + offset_from_unit);
    return LazySymbol();
  }

 private:
  fxl::RefPtr<const SymbolFactory> symbol_factory_;
  DwarfDieRef unit_ref_;
};
