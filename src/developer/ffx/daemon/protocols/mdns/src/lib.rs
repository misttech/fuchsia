...
use mdns_discovery::{
    DiscoveryConfig, MDNS_BROADCAST_INTERVAL, MDNS_INTERFACE_DISCOVERY_INTERVAL, MDNS_TTL,
...
mod tests {
    ...
    async fn test_mdns_get_targets_empty() {
        ...
        let daemon =
            FakeDaemonBuilder::new(&env.context).inject_fidl_protocol::<Mdns>(mdns).build();
