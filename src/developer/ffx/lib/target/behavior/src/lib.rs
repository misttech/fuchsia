...
pub async fn init_connection_behavior(context: &EnvironmentContext) -> Result<ConnectionBehavior> {
...
pub struct FhoTargetEnvironment {
    ...
    behavior: Arc<Mutex<Option<Box<ConnectionBehavior>>>>,
}
...
impl FhoTargetEnvironment {
    ...
    fn maybe_wrap_connection_errors(&self, err: fho::Error) -> fho::Error {
        if let Some(behavior) = self.behavior() {
            if let ConnectionBehavior::DirectConnector(dc) = &behavior
    ...
    pub fn behavior(&self) -> Option<ConnectionBehavior> {
        let b = self.behavior.lock().expect("poisoned behavior lock");
        b.as_ref().map(|boxed| *boxed.clone())
    }

    pub fn set_behavior(&self, new_behavior: ConnectionBehavior) -> Result<()> {
