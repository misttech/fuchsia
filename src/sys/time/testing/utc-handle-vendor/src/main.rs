...
async fn main() -> Result<()> {
    ...
    // Create a new UTC clock handle based off of the current time, and start it.
    let utc_now = fxr::utc_time();
    let boot_now = zx::BootInstant::get();
    let utc_clock = Rc::new(
        UtcClock::create(zx::ClockOpts::BOOT, Some(utc_now)).context("while creating UTC clock")?,
    );
    ...
    log::info!("started the vendored UTC clock");
