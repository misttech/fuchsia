...
use fidl_next_codec::wire::RawWireUnion;
use fidl_next_codec::{
...
use crate::{Flexible, FrameworkError};
...
impl<T: Constrained<Constraint = ()>> Constrained for WireFlexible<'_, T> {
    type Constraint = ();
...
unsafe impl<T> Wire for WireFlexible<'static, T>
where
    T: Wire<Constraint = ()>,
{
    type Narrowed<'de> = WireFlexible<'de, T::Narrowed<'de>>;
...
unsafe impl<'de, D, T> Decode<D> for WireFlexible<'de, T>
where
    D: Decoder<'de> + ?Sized,
    T: Decode<D, Constraint = ()>,
{
    fn decode(
        ...
        match RawWireUnion::encoded_ordinal(raw.as_mut()) {
            ORD_OK => RawWireUnion::decode_as::<D, T>(raw, decoder, constraint)?,
            ORD_FRAMEWORK_ERR => {
...
unsafe impl<E, WT, T> Encode<WireFlexible<'static, WT>, E> for Flexible<T>
where
    E: Encoder + ?Sized,
    WT: Wire<Constraint = ()>,
    T: Encode<WT, E>,
{
    fn encode(
        self,
        encoder: &mut E,
        out: &mut MaybeUninit<WireFlexible<'static, WT>>,
        ...
        match self {
            Self::Ok(value) => {
                RawWireUnion::encode_as::<E, WT>(value, ORD_OK, encoder, raw, constraint)?
            }
            Self::FrameworkErr(error) => RawWireUnion::encode_as::<E, WireFrameworkError>(
                error,
                ORD_FRAMEWORK_ERR,
                encoder,
                raw,
...
unsafe impl<'a, E, WT, T> Encode<WireFlexible<'static, WT>, E> for &'a Flexible<T>
where
    E: Encoder + ?Sized,
    WT: Wire<Constraint = ()>,
    &'a T: Encode<WT, E>,
...
mod tests {
    ...
    fn encode_flexible_result() {
        ...
        assert_eq!(
            Vec::encode(Flexible::<()>::FrameworkErr(FrameworkError::UnknownMethod)).unwrap(),
            chunks![
