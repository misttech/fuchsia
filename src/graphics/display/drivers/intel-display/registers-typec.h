...
#ifndef SRC_GRAPHICS_DISPLAY_DRIVERS_INTEL_DISPLAY_REGISTERS_TYPEC_H_
...
class DynamicFlexIoDisplayPortMainLinkLaneEnabled
 ...
 public:
  ...
  uint32_t enabled_display_port_main_link_lane_bits(intel_display::DdiId ddi_id) const {
    ZX_ASSERT(IsDdiCoveredByThisRegister(ddi_id));
    const uint32_t bit_index = ((ddi_id - intel_display::DdiId::DDI_TC_1) & 0x1) * 4;
  ...
  SelfType& set_enabled_display_port_main_link_lane_bits(intel_display::DdiId ddi_id,
                                                         uint32_t bits) {
    ZX_ASSERT(IsDdiCoveredByThisRegister(ddi_id));
    if (IsSupportedDisplayPortLaneConfig(bits)) {
      const uint32_t lane0_enabled_bit_index =
          ((ddi_id - intel_display::DdiId::DDI_TC_1) & 0x1) * 4;
  ...
  static auto GetForDdi(intel_display::DdiId ddi_id) {
    ZX_ASSERT(ddi_id >= intel_display::DdiId::DDI_TC_1);
    ZX_ASSERT(ddi_id <= intel_display::DdiId::DDI_TC_6);
    const uint32_t fia_index = (ddi_id - intel_display::DdiId::DDI_TC_1) >> 1;
 ...
 private:
  ...
  bool IsDdiCoveredByThisRegister(intel_display::DdiId ddi_id) const {
    switch (reg_addr()) {
      case kFiaOffsets[0]:
        return ddi_id == intel_display::DdiId::DDI_TC_1 || ddi_id == intel_display::DdiId::DDI_TC_2;
      case kFiaOffsets[1]:
        return ddi_id == intel_display::DdiId::DDI_TC_3 || ddi_id == intel_display::DdiId::DDI_TC_4;
      case kFiaOffsets[2]:
        return ddi_id == intel_display::DdiId::DDI_TC_5 || ddi_id == intel_display::DdiId::DDI_TC_6;
      default:
...
class DynamicFlexIoScratchPad : public hwreg::RegisterBase<DynamicFlexIoScratchPad, uint32_t> {
 public:
  ...
  //
  // Callers must make sure they read from the correct FIA register.
  TypeCLiveState type_c_live_state(intel_display::DdiId ddi_id) const {
    ZX_ASSERT(IsDdiCoveredByThisRegister(ddi_id));
    // TODO(https://fxbug.dev/42061535): Move the logic to calculate register bit index
    // from given `ddi_id` to a separate method.
    const uint32_t bit_index = ((ddi_id - intel_display::DdiId::DDI_TC_1) & 0x1) * 8 + 5;
  ...
  //
  // Callers must make sure they read from the correct FIA register.
  uint32_t display_port_tx_lane_assignment(intel_display::DdiId ddi_id) const {
    ZX_ASSERT(IsDdiCoveredByThisRegister(ddi_id));
    const uint32_t bit_index = ((ddi_id - intel_display::DdiId::DDI_TC_1) & 0x1) * 8;
    return hwreg::BitfieldRef<const uint32_t>(reg_value_ptr(), bit_index + 3, bit_index).get();
  }
...
class DynamicFlexIoDisplayPortPinAssignment
 ...
 public:
  ...
  //
  // Callers must make sure they read from the correct FIA register.
  std::optional<PinAssignment> pin_assignment_for_ddi(intel_display::DdiId ddi_id) const {
    ZX_ASSERT(IsDdiCoveredByThisRegister(ddi_id));
    const uint32_t bit_index = ((ddi_id - intel_display::DdiId::DDI_TC_1) & 0x1) * 4;
...
class DynamicFlexIoDisplayPortControllerSafeStateSettings
 ...
 public:
  ...
  SelfType& set_safe_mode_disabled_for_ddi(intel_display::DdiId ddi_id, bool disabled) {
    ZX_ASSERT(IsDdiCoveredByThisRegister(ddi_id));
    const uint32_t bit_index = (ddi_id - intel_display::DdiId::DDI_TC_1) & 0x1;
...
class HipIndexReg0 : public hwreg::RegisterBase<HipIndexReg0, uint32_t> {
 public:
  ...
  SelfType& set_hip_index_for_ddi(intel_display::DdiId ddi_id, uint32_t hip_index) {
    ...
    ZX_ASSERT(ddi_id >= intel_display::DdiId::DDI_TC_1);
    ZX_ASSERT(ddi_id <= intel_display::DdiId::DDI_TC_4);

    const uint32_t bit_low = (ddi_id - intel_display::DdiId::DDI_TC_1) * 8;
...
class HipIndexReg1 : public hwreg::RegisterBase<HipIndexReg1, uint32_t> {
 public:
  ...
  SelfType& set_hip_index_for_ddi(intel_display::DdiId ddi_id, uint32_t hip_index) {
    ZX_ASSERT_MSG((hip_index & (~0b1111)) == 0,
                  "hip_index (0x%x) invalid: it has more than 4 bits.", hip_index);
    ZX_ASSERT(ddi_id >= intel_display::DdiId::DDI_TC_5);
    ZX_ASSERT(ddi_id <= intel_display::DdiId::DDI_TC_6);

    const uint32_t bit_low = (ddi_id - intel_display::DdiId::DDI_TC_5) * 8;
    const uint32_t bit_high = bit_low + 3;
    hwreg::BitfieldRef<uint32_t>(reg_value_ptr(), bit_high, bit_low).set(hip_index);
...
template <typename T>
void WriteHipIndex(T* reg_io, intel_display::DdiId ddi_id, uint32_t hip_index) {
  switch (ddi_id) {
    case intel_display::DdiId::DDI_TC_1:
    case intel_display::DdiId::DDI_TC_2:
    case intel_display::DdiId::DDI_TC_3:
    case intel_display::DdiId::DDI_TC_4: {
      auto hip_index_reg0 = HipIndexReg0::Get().ReadFrom(reg_io);
      hip_index_reg0.set_hip_index_for_ddi(ddi_id, hip_index).WriteTo(reg_io);
      break;
    }
    case intel_display::DdiId::DDI_TC_5:
    case intel_display::DdiId::DDI_TC_6: {
...
class DekelRegisterBase : public ::hwreg::RegisterBase<DerivedType, IntType, PrinterState> {
 public:
  ...
  static uint32_t PhyBaseAddress(intel_display::DdiId ddi_id) {
    ZX_ASSERT(ddi_id >= intel_display::DdiId::DDI_TC_1);
    ZX_ASSERT(ddi_id <= intel_display::DdiId::DDI_TC_6);
    return 0x168000 + (ddi_id - intel_display::DdiId::DDI_TC_1) * 0x1000;
  }
...
class DekelRegisterAddr {
 public:
  DekelRegisterAddr(intel_display::DdiId ddi_id, uint32_t phy_internal_address)
 ...
 private:
  const intel_display::DdiId ddi_id_;
...
class DekelDisplayPortMode : public DekelRegisterBase<DekelDisplayPortMode, uint32_t> {
 public:
  ...
  static auto GetForLaneDdi(uint32_t lane, intel_display::DdiId ddi_id) {
...
#endif  // SRC_GRAPHICS_DISPLAY_DRIVERS_INTEL_DISPLAY_REGISTERS_TYPEC_H_
