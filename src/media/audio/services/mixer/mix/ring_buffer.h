...
class RingBuffer : public std::enable_shared_from_this<RingBuffer> {
 ...
 private:
  ...
  // TODO(https://fxbug.dev/42063096): These must be accessed with atomic instructions
  // (std::atomic_load and std::atomic_store). These can be std::atomic<std::shared_ptr<>> after
  // C++20 is available.
