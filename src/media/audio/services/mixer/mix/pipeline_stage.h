...
class PipelineStage {
 ...
 private:
  ...
  //
  // TODO(https://fxbug.dev/42063096): This must be accessed with atomic instructions
  // (`std::atomic_load` and `std::atomic_store`). This can be `std::atomic<std::shared_ptr<T>>`
  // after C++20 is available.
