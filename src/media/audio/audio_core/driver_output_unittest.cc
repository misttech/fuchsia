...
class TestableDriverOutput : public DriverOutput {
 public:
  template <typename... Args>
  explicit TestableDriverOutput(Args&&... args) : DriverOutput(std::forward<Args>(args)...) {}

  using DriverOutput::mix_domain;
  using DriverOutput::WriteMixOutput;
};
...
class DriverOutputTest : public testing::ThreadingModelFixture {
 protected:
  ...
  void SetUp() override {
    ...
    output_ = std::make_shared<TestableDriverOutput>(
  ...
  zx::duration expected_mix_interval() const { return expected_mix_interval_; }
  VolumeCurve& volume_curve() { return volume_curve_; }
  testing::FakeAudioDriver* driver() { return driver_.get(); }
  std::shared_ptr<TestableDriverOutput>& output() { return output_; }
  fzl::VmoMapper& ring_buffer_mapper() { return ring_buffer_mapper_; }
 ...
 private:
  ...
  std::shared_ptr<TestableDriverOutput> output_;
...
TEST_F(DriverOutputTest, DriverOutputStartsDriver) {
  ...
  threading_model().FidlDomain().ScheduleTask(output()->Startup());
  RunLoopUntilIdle();
  ...
  auto selected_format = driver()->selected_format();
  ...
  threading_model().FidlDomain().ScheduleTask(output()->Shutdown());
  RunLoopUntilIdle();
}
...
TEST_F(DriverOutputTest, RendererOutput) {
  ...
  context().link_matrix().LinkObjects(renderer, output(),
                                      std::make_shared<MappedLoudnessTransform>(volume_curve()));
  ...
  // interval.
  RunLoopFor(expected_mix_interval() - zx::nsec(1));
  const uint32_t kSilentFrame = 0;
  ...
  size_t first_non_silent_frame =
      (supportedSampleFormat.frame_rate * output()->presentation_delay().to_nsecs()) /
      1'000'000'000;
...
TEST_F(DriverOutputTest, MixAtExpectedInterval) {
  ...
  driver()->set_external_delay(kExternalDelay);
  driver()->set_internal_delay(kInternalDelay);
  driver()->set_driver_transfer_bytes(kDriverTransferBytes);
  ConfigureDriverForSampleFormat(supportedSampleFormat);
  ...
  context().link_matrix().LinkObjects(renderer, output(),
                                      std::make_shared<MappedLoudnessTransform>(volume_curve()));
  renderer->EnqueueAudioPacket(0.75, expected_mix_interval());
  renderer->EnqueueAudioPacket(-0.75, expected_mix_interval());
  ...
  // Audio data will be mixed into the ring buffer, offset by exactly that amount.
  auto first_positive_frame_duration =
      output()->presentation_delay() - kInternalDelay - kExternalDelay;
  size_t first_positive_frame =
      (supportedSampleFormat.frame_rate * first_positive_frame_duration.to_nsecs()) / 1'000'000'000;
...
TEST_F(DriverOutputTest, DISABLED_WriteSilenceToRingWhenMuted) {
  ...
  output()->SetGainInfo(gain_info, fuchsia::media::AudioGainValidFlags::MUTE_VALID);
  RunLoopUntilIdle();
  ...
  renderer->EnqueueAudioPacket(1.0, expected_mix_interval(),
                               [&packet1_released] { packet1_released = true; });
  renderer->EnqueueAudioPacket(-1.0, expected_mix_interval(),
                               [&packet2_released] { packet2_released = true; });

  // Fill the ring buffer with some bytes so we can detect if we've written to the buffer.
  memset(ring_buffer_mapper().start(), 0xff, RingBufferSizeBytes());
  ...
  auto first_silent_frame_duration = output()->presentation_delay() - kExternalDelay;
  size_t first_silent_frame =
      (supportedSampleFormat.frame_rate * first_silent_frame_duration.to_nsecs()) / 1'000'000'000;
  size_t num_silent_frames = kMixWindowFrames * 2UL;

  // Run loop to consume all the frames from the renderer.
  RunLoopFor(expected_mix_interval());
  RunLoopFor(expected_mix_interval());
  ...
  // |expected_mix_interval()| - |zx::nsec(1)| to ensure we run the next |Process()| after this lead
  // time has elapsed.
  RunLoopFor(output()->presentation_delay() + expected_mix_interval() - zx::nsec(1));
...
TEST_F(DriverOutputTest, SelectRateAndChannelizationFromDeviceConfig) {
  ...
  EXPECT_EQ(1u, output()->pipeline_config()->root().effects_v1.size());
  EXPECT_EQ(output()->pipeline_config()->root().output_channels, kRequestedDeviceChannels / 2);
  EXPECT_EQ(output()->pipeline_config()->root().output_rate, kRequestedDeviceRate);
  auto format = output()->pipeline_config()->OutputFormat(nullptr);
...
TEST_F(DriverOutputTest, UseBestAvailableSampleRateAndChannelization) {
  ...
  EXPECT_TRUE(output()->pipeline_config()->root().effects_v1.empty());
  EXPECT_EQ(output()->pipeline_config()->root().output_channels, kSupportedChannels);
  EXPECT_EQ(output()->pipeline_config()->root().output_rate, kSupportedFrameRate);
  auto format = output()->pipeline_config()->OutputFormat(nullptr);
  EXPECT_EQ(format.channels(), kSupportedChannels);
  EXPECT_EQ(format.frames_per_second(), kSupportedFrameRate);
}

// Verify synchronization: EnableAudible() and EnableUltrasonic() on a shutting-down DriverOutput.
TEST_F(DriverOutputTest, EnableAudibleAndUltrasonicAfterShutdown) {
  fuchsia::hardware::audio::PcmSupportedFormats formats = {};
  fuchsia::hardware::audio::ChannelSet channel_set = {};
  std::vector<fuchsia::hardware::audio::ChannelAttributes> attributes(kRequestedDeviceChannels / 2);
  for (auto& attr : attributes) {
    attr.set_min_frequency(20);
    attr.set_max_frequency(50000);
  }
  channel_set.set_attributes(std::move(attributes));
  formats.mutable_channel_sets()->push_back(std::move(channel_set));
  formats.mutable_sample_formats()->push_back(fuchsia::hardware::audio::SampleFormat::PCM_SIGNED);
  formats.mutable_bytes_per_sample()->push_back(2);
  formats.mutable_valid_bits_per_sample()->push_back(16);
  formats.mutable_frame_rates()->push_back(48000);
  ConfigureDriverForSampleFormats(std::move(formats));
  ...
  threading_model().FidlDomain().ScheduleTask(output()->Shutdown());
  output()->EnableAudible();
  output()->EnableUltrasonic();
  RunLoopUntilIdle();
}

TEST_F(DriverOutputTest, WriteMixOutputNegativeStartFrameWraparound) {
  fuchsia::hardware::audio::PcmFormat supportedSampleFormat = {
      .number_of_channels = 2,
      .sample_format = fuchsia::hardware::audio::SampleFormat::PCM_SIGNED,
      .bytes_per_sample = 2,
  ...
  // Fill ring buffer with 0xff to verify WriteMixOutput writes silence (0x00).
  memset(ring_buffer_mapper().start(), 0xff, RingBufferSizeBytes());

  // Execute WriteMixOutput on the mix domain with a negative start frame (-10).
  OBTAIN_EXECUTION_DOMAIN_TOKEN(token, &output()->mix_domain());
  output()->WriteMixOutput(-10, 10, nullptr);

  // Verify that the last 10 frames of the ring buffer were filled with 0x00 silence.
  auto total_frames =
      RingBufferSizeBytes() / static_cast<int64_t>(supportedSampleFormat.bytes_per_sample *
                                                   supportedSampleFormat.number_of_channels);
  EXPECT_THAT(RingBufferSlice<uint32_t>(total_frames - 10, 10), Each(Eq(0u)));
}
