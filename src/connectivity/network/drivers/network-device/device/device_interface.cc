...
void TeardownAndFreeFactory(std::unique_ptr<network::NetworkDeviceImplBinder>&& factory) {
  // Keep a raw pointer for calling into since we capture by move in the callback which renders the
  // pointer invalid.
  network::NetworkDeviceImplBinder* factory_ptr = factory.get();
  ...
  // destroyed, thus achieving the same goal.
  factory_ptr->Teardown([factory = std::move(factory)]() mutable { factory.reset(); });
}
...
zx::result<std::unique_ptr<NetworkDeviceInterface>> NetworkDeviceInterface::Create(
    ...
    ddk::NetworkDeviceImplProtocolClient parent, Sys* sys) {
  fbl::AllocChecker ac;
  std::unique_ptr shim = fbl::make_unique_checked<NetworkDeviceShim>(&ac, parent, shim_dispatchers);
  if (!ac.check()) {
    LOGF_ERROR("create: failed to create shim");
    return zx::error(ZX_ERR_NO_MEMORY);
  }

  return NetworkDeviceInterface::Create(dispatchers, std::move(shim), sys);
}

zx::result<std::unique_ptr<NetworkDeviceInterface>> NetworkDeviceInterface::Create(
    const DeviceInterfaceDispatchers& dispatchers,
    std::unique_ptr<NetworkDeviceImplBinder>&& factory, Sys* sys) {
  return internal::DeviceInterface::Create(dispatchers, std::move(factory), sys);
}
...
zx::result<std::unique_ptr<DeviceInterface>> DeviceInterface::Create(
  ...
  if (!ac.check()) {
    TeardownAndFreeFactory(std::move(factory));
    return zx::error(ZX_ERR_NO_MEMORY);
  }

  zx_status_t status = device->Init(std::move(factory));
  if (status != ZX_OK) {
...
zx_status_t DeviceInterface::Init(std::unique_ptr<NetworkDeviceImplBinder>&& factory) {
  ...
  // If init fails the factory has to be torn down. The DeviceInterface::Teardown method is not
  // going to be called at that point but the factory might have state that needs to be torn down in
  // an orderly fashion.
  auto teardown_and_free_factory =
      fit::defer([this] { TeardownAndFreeFactory(std::move(factory_)); });

  zx::result<fdf::ClientEnd<netdriver::NetworkDeviceImpl>> device = factory_->Bind();
  if (device.is_error()) {
  ...
  ifc_binding_ = fdf::BindServer(
      ...
      [this](DeviceInterface*, fidl::UnbindInfo, fdf::ServerEnd<netdriver::NetworkDeviceIfc>) {
        ...
        ifc_binding_.reset();
        if (teardown_state_ == TeardownState::IFC_BINDING) {
  ...
  if (!init_status.ok()) {
    LOGF_ERROR("init: Init() failed: %s", init_status.status_string());
    return init_status.status();
  }
  // Now that everything succeeded do NOT tear down the factory.
  teardown_and_free_factory.cancel();
...
void DeviceInterface::AddPort(netdriver::wire::NetworkDeviceIfcAddPortRequest* request,
  ...
  DevicePort::Create(
      this, dispatchers_.impl_->async_dispatcher(), salted_id, std::move(port_client),
...
void DeviceInterface::Snoop(fuchsia_hardware_network_driver::wire::NetworkDeviceIfcSnoopRequest*,
  ...
  // TODO(fxbug.dev/43028): Implement real version. Current implementation acts as if no LISTEN
  // is ever in place.
}
  ...
  fit::callback<void()> teardown_callback =
    ...
    switch (teardown_state_) {
      ...
      case TeardownState::IFC_BINDING:
        ...
        teardown_state_ = TeardownState::FACTORY;
        if (factory_) {
          NetworkDeviceImplBinder::Synchronicity synchronicity = factory_->Teardown([this] {
            control_lock_.Acquire();
            ContinueTeardown(TeardownState::FACTORY);
          });
          if (synchronicity == NetworkDeviceImplBinder::Synchronicity::Async) {
            // The teardown of the factory will complete asynchronously, the callback will trigger
            // the transition to the next state.
            LOG_TRACE("teardown: async teardown of factory");
            return nullptr;
          }
          // The teardown complete synchronously, continue on to the next state.
        }
        // There was no factory or teardown is already complete, move immediately to the next step.
        __FALLTHROUGH;
      case TeardownState::FACTORY:
        LOG_TRACE("teardown state is FACTORY");
  ...
  while (it != dead_sessions_.end()) {
    ...
    if (session.ShouldDestroy()) {
      ...
      async::PostTask(dispatchers_.impl_->async_dispatcher(), [&session, this]() {
        ...
        ReleaseVmo(session, [this, session = &session] {
          control_lock_.Acquire();
