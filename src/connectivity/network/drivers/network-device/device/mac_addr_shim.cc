...
void MacAddrShim::Bind(const fdf::Dispatcher& dispatcher, ddk::MacAddrProtocolClient client_impl,
  ...
  fdf::ServerBindingRef binding_ref =
      fdf::BindServer(dispatcher.get(), std::move(server_end), std::move(impl));
  impl_ptr->binding_.emplace(std::move(binding_ref));
}

MacAddrShim::MacAddrShim(ddk::MacAddrProtocolClient impl) : impl_(impl) {}

void MacAddrShim::SetMode(fuchsia_hardware_network_driver::wire::MacAddrSetModeRequest* request,
...
void MacAddrShim::GetFeatures(fdf::Arena& arena, GetFeaturesCompleter::Sync& completer) {
  ...
  fidl::WireTableBuilder builder = fuchsia_hardware_network_driver::wire::Features::Builder(arena);

  builder.multicast_filter_count(features.multicast_filter_count)
      .supported_modes(
          fuchsia_hardware_network_driver::wire::SupportedMacFilterMode(features.supported_modes));
