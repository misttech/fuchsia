...
void NetworkPortShim::Bind(ddk::NetworkPortProtocolClient client_impl,
  ...
  fdf::ServerBindingRef binding_ref =
      fdf::BindServer(impl->dispatcher_->get(), std::move(server_end), std::move(impl));
  impl_ptr->binding_.emplace(std::move(binding_ref));
}

NetworkPortShim::NetworkPortShim(ddk::NetworkPortProtocolClient impl,
                                 const fdf::Dispatcher* dispatcher)
