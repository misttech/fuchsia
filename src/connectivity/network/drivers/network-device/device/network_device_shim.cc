...
void NetworkDeviceShim::Init(netdriver::wire::NetworkDeviceImplInitRequest* request,
  ...
  device_ifc_ = fdf::WireSharedClient<netdriver::NetworkDeviceIfc>(std::move(request->iface),
                                                                   dispatchers_.shim_->get());
  completer.buffer(arena).Reply(impl_.Init(this, &network_device_ifc_protocol_ops_));
}
...
void NetworkDeviceShim::QueueTx(netdriver::wire::NetworkDeviceImplQueueTxRequest* request,
  ...
  tx_buffer_t buffers[request->buffers.count()];
  buffer_region_t regions[request->buffers.count() * netdriver::kMaxBufferParts];
...
void NetworkDeviceShim::QueueRxSpace(netdriver::wire::NetworkDeviceImplQueueRxSpaceRequest* request,
  ...
  rx_space_buffer_t buffers[request->buffers.count()];
  for (size_t i = 0; i < request->buffers.count(); ++i) {
...
zx_status_t NetworkDeviceShim::NetworkDeviceIfcAddPort(uint8_t port_id,
  ...
  NetworkPortShim::Bind(impl, dispatchers_.port_, std::move(endpoints->server));
  ...
  // The sync call below is not guaranteed to run on the dispatcher associated with device_. It will
  // most likely run on whatever the current dispatcher is. Since that will most likely be a
  // dispatcher from the driver calling AddPort there is no knowledge of what type of dispatcher it
  // is here. If that driver does not support sync calls then the sync call below will fail. Work
  // around this by making an asynchronous call if needed and waiting for the result. Unfortunately
  // if we are on the same dispatcher as device_ is associated with then the asynchronous approach
  // will deadlock because the dispatcher would then wait for itself to process a call it can't
  // process because it's waiting for itself.
  //
  // This is necessary because AddPort has to be a synchronous call from a banjo perspective, a
  // driver could otherwise attempt operations on a port that would fail because the creation has
  // not yet completed.
  if (fdf::Dispatcher::GetCurrent()->get() != dispatchers_.shim_->get()) {
    // We're on the wrong dispatcher, post a task to make a sync call on the correct dispatcher and
  ...
  // We're on device_'s dispatcher, making a synchronous call to avoid deadlock.
  fdf::WireUnownedResult result =
      device_ifc_.sync().buffer(arena)->AddPort(port_id, std::move(endpoints->client));
  if (!result.ok()) {
    LOGF_ERROR("AddPort failed: %s", result.FormatDescription().c_str());
