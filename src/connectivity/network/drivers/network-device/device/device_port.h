...
class DevicePort : public fidl::WireServer<netdev::Port> {
 public:
  ...
  // on an error the callback may be (but is not guaranteed to be) called recursively from the
  // Create call. Because of this it's a good idea to avoid acquiring locks in the on_created
