...
FakeMacDeviceFidlImpl::FakeMacDeviceFidlImpl()
...
fdf::ClientEnd<fuchsia_hardware_network_driver::MacAddr> FakeMacDeviceFidlImpl::Bind(
...
zx::result<std::unique_ptr<MacAddrDeviceInterface>> FakeMacDeviceFidlImpl::CreateChild(
    const fdf::Dispatcher& dispatcher) {
  auto endpoints = fdf::CreateEndpoints<fuchsia_hardware_network_driver::MacAddr>();
...
void FakeMacDeviceFidlImpl::GetAddress(fdf::Arena& arena, GetAddressCompleter::Sync& completer) {
  completer.buffer(arena).Reply(mac_);
}

void FakeMacDeviceFidlImpl::GetFeatures(fdf::Arena& arena, GetFeaturesCompleter::Sync& completer) {
...
void FakeMacDeviceFidlImpl::SetMode(
...
zx_status_t FakeMacDeviceFidlImpl::WaitConfigurationChanged() {
  zx_status_t status = event_.wait_one(kConfigurationChangedEvent, zx::time::infinite(), nullptr);
  event_.signal(kConfigurationChangedEvent, 0);
  return status;
}
