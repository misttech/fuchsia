...
const DnsName kLocalHostFullNameLong = DnsName("testhostnamethatislong.local");
const DnsName kAltHostName = DnsName("123456789ABC");
const DnsName kAltHostFullName = DnsName("123456789ABC.local");
...
class InstanceResponderTest : public AgentTest, public Mdns::Publisher {
 ...
 protected:
  static DnsName service_full_name() { return MdnsNames::ServiceFullName(kServiceName); }

  static DnsName instance_full_name() {
  ...
  fit::function<void(std::unique_ptr<Mdns::Publication>)> ExpectGetPublicationCall(
      PublicationCause publication_cause, const DnsLabel& subtype,
...
TEST_F(InstanceResponderTest, Startup) {
  InstanceResponder under_test(this, DnsName(), {}, kServiceName, kInstanceName, Media::kBoth,
...
TEST_F(InstanceResponderTest, WirelessOnly) {
  InstanceResponder under_test(this, DnsName(), {}, kServiceName, kInstanceName, Media::kWireless,
                               IpVersions::kBoth, this);
  SetAgent(under_test);
  ...
  ExpectGetPublicationCall(
      PublicationCause::kQueryMulticastResponse, DnsLabel(),
...
TEST_F(InstanceResponderTest, WiredOnly) {
  InstanceResponder under_test(this, DnsName(), {}, kServiceName, kInstanceName, Media::kWired,
                               IpVersions::kBoth, this);
  SetAgent(under_test);
...
TEST_F(InstanceResponderTest, Unicast) {
  ...
  ExpectGetPublicationCall(PublicationCause::kQueryUnicastResponse, DnsLabel(),
                           {sender_address.socket_address()})(Mdns::Publication::Create(kPort));
...
TEST_F(InstanceResponderTest, Subtype) {
  ...
  under_test.ReceiveQuestion(
      DnsQuestion(DnsName("_cookies._sub").append(service_full_name()), DnsType::kPtr),
      ReplyAddress::Multicast(Media::kBoth, IpVersions::kBoth), sender_address);
