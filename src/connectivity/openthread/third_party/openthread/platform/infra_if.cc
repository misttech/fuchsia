...
#ifdef OPENTHREAD_260323
extern "C" {
bool otPlatInfraIfHasAddress(otInstance *a_instance, uint32_t a_infra_if_index,
                             const otIp6Address *a_address) {
  OT_UNUSED_VARIABLE(a_instance);
#endif
#ifdef OPENTHREAD_251001
...
#ifdef OPENTHREAD_260323
  otError otPlatInfraIfSendIcmp6Nd(otInstance * a_instance, uint32_t a_infra_if_index,
                                   const otIp6Address *a_dest_address, const uint8_t *a_buffer,
                                   uint16_t a_buffer_length) {
#endif
#ifdef OPENTHREAD_251001
  ...
  int CreateIcmp6Socket(void) {
    ...
    otPlatLog(OT_LOG_LEVEL_WARN, OT_LOG_REGION_PLATFORM,
              "IPPROTO_ICMPV6, ICMP6_FILTER ret_val:%d, errno:%s", rval, strerror(errno));
    ...
    otPlatLog(OT_LOG_LEVEL_WARN, OT_LOG_REGION_PLATFORM,
              "IPPROTO_IPV6, IPV6_UNICAST_HOPS ret_val:%d, errno:%s", rval, strerror(errno));
    VERIFY_OR_ASSERT(rval == 0, "OT_EXIT_ERROR_ERRNO");

    rval = setsockopt(sock, IPPROTO_IPV6, IPV6_MULTICAST_HOPS, &kHopLimit, sizeof(kHopLimit));
    otPlatLog(OT_LOG_LEVEL_WARN, OT_LOG_REGION_PLATFORM,
              "IPPROTO_IPV6, IPV6_MULTICAST_HOPS ret_val:%d, errno:%s", rval, strerror(errno));
    VERIFY_OR_ASSERT(rval == 0, "OT_EXIT_ERROR_ERRNO");
  ...
  void InfraNetif::Init(uint32_t infra_if_idx) {
    ...
    otPlatLog(OT_LOG_LEVEL_INFO, OT_LOG_REGION_PLATFORM,
              "InfraNetif::Init() intf found: infra_if_name_:%s, strlen:%zu, size:%zu",
              infra_if_name_, strlen(infra_if_name_), sizeof(*(infra_if_name_)));
    infra_if_name_len_ = strlen(infra_if_name_);

    otPlatLog(OT_LOG_LEVEL_INFO, OT_LOG_REGION_PLATFORM, "InfraNetif::Init() infra_if_name_len_:%u",
              infra_if_name_len_);
    ...
    otPlatLog(OT_LOG_LEVEL_INFO, OT_LOG_REGION_PLATFORM,
              "InfraNetif::Init() SOL_SOCKET, SO_BINDTODEVICE rval:%zd, errno:%s", rval,
              strerror(errno));
    VERIFY_OR_ASSERT(rval == 0, "OT_EXIT_ERROR_ERRNO");
    otPlatLog(OT_LOG_LEVEL_INFO, OT_LOG_REGION_PLATFORM,
  ...
  void InfraNetif::ReceiveIcmp6Message(otInstance *a_instance) {
    ...
    a_instance:x%lx, infra_if_idx_:%u, src_addr:[%.2x%.2x:%.2x%.2x,..], buffer:[%u,%u,%u,%u,..], buf_len:%u",
