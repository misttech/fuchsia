...
#define IWL_EFI_VAR_GUID                                                   \
	EFI_GUID(0x92daaf2f, 0xc02b, 0x455b, 0xb2, 0xec, 0xf5, 0xa3, 0x59, \
		 0x4f, 0x4a, 0xea)
					   ...
					   const u8 *data, size_t len)
{
	...
	while (len >= sizeof(*tlv)) {
		...
		case IWL_UCODE_TLV_MEM_DESC: {
			...
			tmp = krealloc(reduce_power_data, size + tlv_len,
				       GFP_KERNEL);
			if (!tmp) {
				IWL_DEBUG_FW(
					trans,
	...
	while (len >= sizeof(*tlv)) {
		...
		if (tlv_type == IWL_UCODE_TLV_PNVM_SKU) {
			...
			if (trans->sku_id[0] == le32_to_cpu(sku_id->data[0]) &&
				...
				sec_data = iwl_uefi_reduce_power_section(
					trans, data, len);
				if (!IS_ERR(sec_data))
					return sec_data;
			} else {
...
void *iwl_uefi_get_reduced_power(struct iwl_trans *trans, size_t *len)
{
	...
	*len = 0;
	...
	memcpy(&reduce_power_efivar->var.VariableName,
	...
	err = efivar_entry_get(reduce_power_efivar, NULL, &package_size,
			       package);
	if (err) {
		IWL_DEBUG_FW(
			trans,
	...
	*len = package_size;
	...
	IWL_DEBUG_FW(trans, "rev %d, total_size %d, n_skus %d\n", package->rev,
		     package->total_size, package->n_skus);
			     ...
			     struct iwl_fw_runtime *fwrt)
{
	...
	if (err) {
		IWL_DEBUG_FW(trans, "SGOM UEFI variable not found %d\n", err);
		goto out_free;
	}
