...
const guid_t iwl_rfi_guid = GUID_INIT(0x7266172C, 0x220B, 0x4B29, 0x81, 0x4F,
				      0x75, 0xE4, 0xDD, 0x26, 0xB5, 0xFD);
			       ...
			       acpi_handle *ret_handle)
{
	...
	if (!root_handle) {
		IWL_DEBUG_DEV_RADIO(
			dev, "ACPI: Could not retrieve root port handle\n");
		return -ENOENT;
	}

	status = acpi_get_handle(root_handle, method, ret_handle);
	if (ACPI_FAILURE(status)) {
		IWL_DEBUG_DEV_RADIO(dev, "ACPI: %s method not found\n", method);
		return -ENOENT;
	}
	return 0;
}
				     ...
				     const guid_t *guid)
{
	...
	obj = acpi_evaluate_dsm(ACPI_HANDLE(dev), guid, rev, func, args);
	if (!obj) {
		IWL_DEBUG_DEV_RADIO(
			dev,
				    ...
				    size_t expected_size)
{
	...
	if (IS_ERR(obj)) {
		IWL_DEBUG_DEV_RADIO(
			dev, "Failed to get  DSM object. func= %d\n", func);
		return -ENOENT;
	}
			...
			const guid_t *guid, u8 *value)
{
	...
	ret = iwl_acpi_get_dsm_integer(dev, rev, func, guid, &val, sizeof(u8));
			 ...
			 const guid_t *guid, u32 *value)
{
	...
	ret = iwl_acpi_get_dsm_integer(dev, rev, func, guid, &val, sizeof(u32));
...
int iwl_acpi_get_tas(struct iwl_fw_runtime *fwrt, union iwl_tas_config_cmd *cmd,
		     int fw_ver)
{
	...
	wifi_pkg = iwl_acpi_get_wifi_pkg(fwrt->dev, data,
					 ACPI_WTAS_WIFI_DATA_SIZE, &tbl_rev);
	if (IS_ERR(wifi_pkg)) {
	...
	if (tbl_rev == 1 &&
		...
		u16 override_iec =
			(tas_selection & ACPI_WTAS_OVERRIDE_IEC_MSK) >>
			ACPI_WTAS_OVERRIDE_IEC_POS;
		u16 enabled_iec = (tas_selection & ACPI_WTAS_ENABLE_IEC_MSK) >>
				  ACPI_WTAS_ENABLE_IEC_POS;
		u8 usa_tas_uhb = (tas_selection & ACPI_WTAS_USA_UHB_MSK) >>
				 ACPI_WTAS_USA_UHB_POS;
...
static int iwl_sar_set_profile(union acpi_object *table,
			       struct iwl_sar_profile *profile, bool enabled,
...
static int iwl_sar_fill_table(struct iwl_fw_runtime *fwrt, __le16 *per_chain,
			      u32 n_subbands, int prof_a, int prof_b)
{
	...
	for (i = 0; i < ACPI_SAR_NUM_CHAINS_REV0; i++) {
		...
		IWL_DEBUG_INFO(fwrt, "SAR EWRD: chain %d profile index %d\n", i,
			       profs[i]);
...
int iwl_sar_select_profile(struct iwl_fw_runtime *fwrt, __le16 *per_chain,
			   u32 n_tables, u32 n_subbands, int prof_a, int prof_b)
{
	int i, ret = 0;

	for (i = 0; i < n_tables; i++) {
		ret = iwl_sar_fill_table(
			fwrt,
...
int iwl_sar_get_wrds_table(struct iwl_fw_runtime *fwrt)
{
	...
	wifi_pkg = iwl_acpi_get_wifi_pkg(
		fwrt->dev, data, ACPI_WRDS_WIFI_DATA_SIZE_REV2, &tbl_rev);
	if (!IS_ERR(wifi_pkg)) {
		if (tbl_rev != 2) {
			ret = PTR_ERR(wifi_pkg);
			goto out_free;
	...
	/* then try revision 1 */
	wifi_pkg = iwl_acpi_get_wifi_pkg(
		fwrt->dev, data, ACPI_WRDS_WIFI_DATA_SIZE_REV1, &tbl_rev);
	if (!IS_ERR(wifi_pkg)) {
		if (tbl_rev != 1) {
			ret = PTR_ERR(wifi_pkg);
			goto out_free;
	...
	wifi_pkg = iwl_acpi_get_wifi_pkg(
		fwrt->dev, data, ACPI_WRDS_WIFI_DATA_SIZE_REV0, &tbl_rev);
	if (!IS_ERR(wifi_pkg)) {
		if (tbl_rev != 0) {
			ret = PTR_ERR(wifi_pkg);
			goto out_free;
...
read_table:
	...
	ret = iwl_sar_set_profile(table, &fwrt->sar_profiles[0],
				  flags & IWL_SAR_ENABLE_MSK, num_chains,
				  num_sub_bands);
out_free:
	kfree(data);
	return ret;
...
int iwl_sar_get_ewrd_table(struct iwl_fw_runtime *fwrt)
{
	...
	/* start by trying to read revision 2 */
	wifi_pkg = iwl_acpi_get_wifi_pkg(
		fwrt->dev, data, ACPI_EWRD_WIFI_DATA_SIZE_REV2, &tbl_rev);
	if (!IS_ERR(wifi_pkg)) {
		if (tbl_rev != 2) {
			ret = PTR_ERR(wifi_pkg);
			goto out_free;
	...
	/* then try revision 1 */
	wifi_pkg = iwl_acpi_get_wifi_pkg(
		fwrt->dev, data, ACPI_EWRD_WIFI_DATA_SIZE_REV1, &tbl_rev);
	if (!IS_ERR(wifi_pkg)) {
		if (tbl_rev != 1) {
			ret = PTR_ERR(wifi_pkg);
			goto out_free;
	...
	/* then finally revision 0 */
	wifi_pkg = iwl_acpi_get_wifi_pkg(
		fwrt->dev, data, ACPI_EWRD_WIFI_DATA_SIZE_REV0, &tbl_rev);
	if (!IS_ERR(wifi_pkg)) {
		if (tbl_rev != 0) {
			ret = PTR_ERR(wifi_pkg);
			goto out_free;
...
int iwl_sar_get_wgds_table(struct iwl_fw_runtime *fwrt)
{
	...
	for (idx = 0; idx < ARRAY_SIZE(rev_data); idx++) {
		...
		u32 profile_size =
			ACPI_GEO_PER_CHAIN_SIZE * rev_data[idx].bands;
		...
		wifi_pkg = iwl_acpi_get_wifi_pkg_range(
			fwrt->dev, data, min_size, max_size, &tbl_rev);
		if (!IS_ERR(wifi_pkg)) {
	...
	for (i = 0; i < n_profiles; i++) {
		for (j = 0; j < n_bands; j++) {
			...
			IWL_DEBUG_RADIO(
				...
				i, j, fwrt->geo_profiles[i].bands[j].chains[0],
				fwrt->geo_profiles[i].bands[j].chains[1],
				fwrt->geo_profiles[i].bands[j].max);
		}
	}
...
__le32 iwl_acpi_get_lari_config_bitmap(struct iwl_fw_runtime *fwrt)
{
	 ...
	 */
	ret = iwl_acpi_get_dsm_u8(fwrt->dev, 0, DSM_FUNC_ENABLE_INDONESIA_5G2,
				  &iwl_guid, &value);
	 ...
	 */
	ret = iwl_acpi_get_dsm_u8(fwrt->dev, 0, DSM_FUNC_DISABLE_SRD, &iwl_guid,
				  &value);
	if (!ret) {
		if (value == DSM_VALUE_SRD_PASSIVE)
			config_bitmap |= cpu_to_le32(
...
read_table:
	...
	for (i = 0; i < IWL_NUM_CHAIN_LIMITS; i++) {
		for (j = 0; j < num_sub_bands; j++) {
			...
			if ((j == 0 && (fwrt->ppag_chains[i].subbands[j] >
						ACPI_PPAG_MAX_LB ||
					fwrt->ppag_chains[i].subbands[j] <
						ACPI_PPAG_MIN_LB)) ||
			    (j != 0 && (fwrt->ppag_chains[i].subbands[j] >
						ACPI_PPAG_MAX_HB ||
					fwrt->ppag_chains[i].subbands[j] <
						ACPI_PPAG_MIN_HB))) {
				fwrt->ppag_flags = 0;
...
int iwl_read_ppag_table(struct iwl_fw_runtime *fwrt,
			union iwl_ppag_table_cmd *cmd, int *cmd_size)
{
	...
	cmd_ver = iwl_fw_lookup_cmd_ver(
		fwrt->fw, WIDE_ID(PHY_OPS_GROUP, PER_PLATFORM_ANT_GAIN_CMD),
