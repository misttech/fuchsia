...
static const struct iwl_rate_mcs_info rate_mcs[IWL_RATE_COUNT] = {
	{ "1", "BPSK DSSS" },  { "2", "QPSK DSSS" },  { "5.5", "BPSK CCK" },
	{ "11", "QPSK CCK" },  { "6", "BPSK 1/2" },   { "9", "BPSK 1/2" },
	{ "12", "QPSK 1/2" },  { "18", "QPSK 3/4" },  { "24", "16QAM 1/2" },
	{ "36", "16QAM 3/4" }, { "48", "64QAM 2/3" }, { "54", "64QAM 3/4" },
	{ "60", "64QAM 5/6" },
};
...
static const char *const pretty_bw[] = {
	"20Mhz", "40Mhz", "80Mhz", "160 Mhz", "320Mhz",
};
...
u32 iwl_new_rate_from_v1(u32 rate_v1)
{
	...
	if (rate_v1 & RATE_MCS_HT_MSK_V1) {
		...
		rate_v2 |= rate_v1 & RATE_HT_MCS_RATE_CODE_MSK_V1;
	...
	rate_v2 |= (rate_v1 & RATE_MCS_CHAN_WIDTH_MSK_V1) |
		   ...
		   (rate_v1 & RATE_MCS_STBC_MSK) | (rate_v1 & RATE_MCS_BF_MSK);
...
int rs_pretty_print_rate(char *buf, int bufsz, const u32 rate)
{
	...
	u32 bw = (rate & RATE_MCS_CHAN_WIDTH_MSK) >> RATE_MCS_CHAN_WIDTH_POS;
	...
	mcs = format == RATE_MCS_HT_MSK ? RATE_HT_MCS_INDEX(rate) :
					  rate & RATE_MCS_CODE_MSK;
	nss = ((rate & RATE_MCS_NSS_MSK) >> RATE_MCS_NSS_POS) + 1;
	sgi = format == RATE_MCS_HE_MSK ? iwl_he_is_sgi(rate) :
					  rate & RATE_MCS_SGI_MSK;

	return scnprintf(buf, bufsz,
			 "0x%x: %s | ANT: %s BW: %s MCS: %d NSS: %d %s%s%s%s%s",
			 rate, type, iwl_rs_pretty_ant(ant),
			 iwl_rs_pretty_bw(bw), mcs, nss,
			 (sgi) ? "SGI " : "NGI ",
...
bool iwl_he_is_sgi(u32 rate_n_flags)
{
	...
	if (type == RATE_MCS_HE_TYPE_SU || type == RATE_MCS_HE_TYPE_EXT_SU)
