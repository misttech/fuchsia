				   ...
				   size_t len)
{
	...
	while (len >= sizeof(*tlv)) {
		...
		case IWL_UCODE_TLV_HW_TYPE:
			...
			rf_id = le16_to_cpup(
				(const __le16 *)(data + sizeof(__le16)));
		...
		case IWL_UCODE_TLV_SEC_RT: {
			const struct iwl_pnvm_section *section =
				(const void *)data;
			u32 data_len = tlv_len - sizeof(*section);

			IWL_DEBUG_FW(trans, "Got IWL_UCODE_TLV_SEC_RT len %d\n",
				     tlv_len);
...
static int iwl_pnvm_parse(struct iwl_trans *trans, const u8 *data, size_t len)
{
	const struct iwl_ucode_tlv *tlv;
...
skip_parse:
	...
	if (ret)
		IWL_DEBUG_FW(trans, "Failed to set reduce power table %d\n",
			     ret);
	kfree(data);

skip_reduce_power:
	iwl_init_notification_wait(notif_wait, &pnvm_wait, ntf_cmds,
				   ARRAY_SIZE(ntf_cmds), iwl_pnvm_complete_fn,
				   trans);
