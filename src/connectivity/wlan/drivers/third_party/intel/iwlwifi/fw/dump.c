...
static void iwl_fwrt_dump_umac_error_log(struct iwl_fw_runtime *fwrt)
{
	...
	if (!base && !(fwrt->trans->dbg.error_event_table_tlv_status &
...
static void iwl_fwrt_dump_lmac_error_log(struct iwl_fw_runtime *fwrt,
					 u8 lmac_num)
{
	struct iwl_trans *trans = fwrt->trans;
	...
	if (base < 0x400000) {
		IWL_ERR(fwrt,
			...
			base,
			(fwrt->cur_fw_img == IWL_UCODE_INIT) ? "Init" : "RT");
		return;
	}
