...
#define FWRT_DEBUGFS_READ_WRAPPER(name)                                     \
	...
	{                                                                   \
		...
		if (!data->read_done) {                                     \
			data->read_done = true;                             \
			data->rlen = iwl_dbgfs_##name##_read(               \
				data->arg, sizeof(data->rbuf), data->rbuf); \
		}                                                           \
                                                                            \
		if (data->rlen < 0)                                         \
			return data->rlen;                                  \
...
#define FWRT_DEBUGFS_WRITE_WRAPPER(name, buflen, argtype)                     \
	...
	{                                                                     \
		argtype *arg =                                                \
			((struct dbgfs_##name##_data *)file->private_data)    \
				->arg;                                        \
		char buf[buflen] = {};                                        \
...
static void *iwl_dbgfs_fw_info_seq_next(struct seq_file *seq, void *v,
					loff_t *pos)
{
...
static void iwl_dbgfs_fw_info_seq_stop(struct seq_file *seq, void *v)
{
	kfree(v);
}
...
static int iwl_dbgfs_fw_info_open(struct inode *inode, struct file *filp)
{
	...
	priv = __seq_open_private(filp, &iwl_dbgfs_info_seq_ops, sizeof(*priv));
