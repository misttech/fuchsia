...
static int
rs_fw_vht_highest_rx_mcs_index(const struct ieee80211_sta_vht_cap *vht_cap,
			       int nss)
{
	...
	for (i = 0; i < max_nss && i < IWL_TLC_NSS_MAX; i++) {
		...
		if (sta->deflink.bandwidth == IEEE80211_STA_RX_BW_160 &&
		    ieee80211_get_vht_max_nss(&ieee_vht_cap,
					      IEEE80211_VHT_CHANWIDTH_160MHZ, 0,
					      true, nss) >= nss)
				       ...
				       struct iwl_tlc_config_cmd_v4 *cmd)
{
	...
	u16 tx_mcs_80 = le16_to_cpu(
		sband->iftype_data->he_cap.he_mcs_nss_supp.tx_mcs_80);
	u16 tx_mcs_160 = le16_to_cpu(
		sband->iftype_data->he_cap.he_mcs_nss_supp.tx_mcs_160);
	int i;
		     ...
		     enum nl80211_band band, bool update)
{
	...
	struct iwl_tlc_config_cmd_v4 cfg_cmd = {
		...
		.max_ch_width = update ? rs_fw_bw_from_sta_bw(sta) :
	...
	cmd_ver = iwl_fw_lookup_cmd_ver(
		mvm->fw, WIDE_ID(DATA_PATH_GROUP, TLC_MNG_CONFIG_CMD), 0);
	IWL_DEBUG_RATE(mvm,
	...
	IWL_DEBUG_RATE(
		mvm, "TLC CONFIG CMD, mpdu_len=%d, no_ht_rate=0x%X, tx_op=%d\n",
		cfg_cmd.max_mpdu_len, cfg_cmd.non_ht_rates, cfg_cmd.max_tx_op);
	IWL_DEBUG_RATE(
		mvm, "TLC CONFIG CMD, ht_rate[0][0]=0x%X, ht_rate[1][0]=0x%X\n",
		cfg_cmd.ht_rates[0][0], cfg_cmd.ht_rates[1][0]);
	IWL_DEBUG_RATE(
		mvm, "TLC CONFIG CMD, ht_rate[0][1]=0x%X, ht_rate[1][1]=0x%X\n",
		cfg_cmd.ht_rates[0][1], cfg_cmd.ht_rates[1][1]);
	IWL_DEBUG_RATE(
		mvm, "TLC CONFIG CMD, ht_rate[0][2]=0x%X, ht_rate[1][2]=0x%X\n",
