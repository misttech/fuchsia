...
void iwl_mvm_teardown_tdls_peers(struct iwl_mvm *mvm)
{
	...
	for (i = 0; i < mvm->fw->ucode_capa.num_stations; i++) {
		...
		ieee80211_tdls_oper_request(
			mvmsta->vif, sta->addr, NL80211_TDLS_TEARDOWN,
...
static int iwl_mvm_tdls_config_channel_switch(
	struct iwl_mvm *mvm, struct ieee80211_vif *vif,
	enum iwl_tdls_channel_switch_type type, const u8 *peer,
	bool peer_initiator, u8 oper_class, struct cfg80211_chan_def *chandef,
	u32 timestamp, u16 switch_time, u16 switch_timeout, struct sk_buff *skb,
	u32 ch_sw_tm_ie)
{
	struct ieee80211_sta *sta;
	struct iwl_mvm_sta *mvmsta;
	...
	tail->timing.max_offchan_duration = cpu_to_le32(
		TU_TO_US(vif->bss_conf.dtim_period * vif->bss_conf.beacon_int) /
		2);
	...
	iwl_mvm_set_tx_cmd(mvm, skb, &tail->frame.tx_cmd, info, mvmsta->sta_id);
...
void iwl_mvm_tdls_ch_switch_work(struct work_struct *work)
{
	...
	ret = iwl_mvm_tdls_config_channel_switch(
		mvm, vif, TDLS_SEND_CHAN_SW_REQ, sta->addr,
		mvm->tdls_cs.peer.initiator, mvm->tdls_cs.peer.op_class,
		&mvm->tdls_cs.peer.chandef, 0, 0, 0, mvm->tdls_cs.peer.skb,
				...
				struct sk_buff *tmpl_skb, u32 ch_sw_tm_ie)
{
	...
	ret = iwl_mvm_tdls_config_channel_switch(
		mvm, vif, TDLS_SEND_CHAN_SW_REQ, sta->addr, sta->tdls_initiator,
		oper_class, chandef, 0, 0, 0, tmpl_skb, ch_sw_tm_ie);
	if (ret)
		goto out;
	 ...
	 */
	delay = 2 *
		TU_TO_MS(vif->bss_conf.dtim_period * vif->bss_conf.beacon_int);
	mod_delayed_work(system_wq, &mvm->tdls_cs.dwork,
			 msecs_to_jiffies(delay));
