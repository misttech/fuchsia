...
void iwl_mvm_ipv6_addr_change(struct ieee80211_hw *hw,
			      struct ieee80211_vif *vif, struct inet6_dev *idev)
{
	struct iwl_mvm_vif *mvmvif = iwl_mvm_vif_from_mac80211(vif);
...
static const u8 *iwl_mvm_find_max_pn(struct ieee80211_key_conf *key,
				     ...
				     struct ieee80211_key_seq *seq, int tid,
				     int queues)
{
					...
					void *_data)
{
	...
	case WLAN_CIPHER_SUITE_WEP104: { /* hack it for now */
		struct {
			...
			.wep_key_cmd.mac_id_n_color = cpu_to_le32(
					    ...
					    void *_data)
{
	...
	case WLAN_CIPHER_SUITE_TKIP:
		if (sta) {
			...
			tkip_sc = data->rsc_tsc->params.all_tsc_rsc.tkip
					  .unicast_rsc;
			tkip_tx_sc =
	...
	case WLAN_CIPHER_SUITE_GCMP_256:
		if (sta) {
			...
			aes_sc = data->rsc_tsc->params.all_tsc_rsc.aes
					 .unicast_rsc;
			aes_tx_sc = &data->rsc_tsc->params.all_tsc_rsc.aes.tsc;
					 ...
					 struct ieee80211_vif *vif)
{
	...
	if (ver == 5) {
		...
		ieee80211_iter_keys(mvm->hw, vif,
				    iwl_mvm_wowlan_get_rsc_v5_data, &data);
		...
		ieee80211_iter_keys(mvm->hw, vif,
				    iwl_mvm_wowlan_get_rsc_tsc_data, &data);
					 ...
					 void *_data)
{
	...
	case WLAN_CIPHER_SUITE_TKIP:
		...
		ieee80211_get_tkip_rx_p1k(key, vif->bss_conf.bssid, cur_rx_iv32,
					  p1k);
				    ...
				    struct cfg80211_wowlan *wowlan)
{
	...
	for (i = 0; i < wowlan->n_patterns; i++) {
		...
		memcpy(&pattern_cmd->patterns[i].mask, wowlan->patterns[i].mask,
		       mask_len);
...
static int iwl_mvm_send_patterns(struct iwl_mvm *mvm, struct ieee80211_vif *vif,
				 struct cfg80211_wowlan *wowlan)
{
	struct iwl_mvm_vif *mvmvif = iwl_mvm_vif_from_mac80211(vif);
	...
	int i, err;
	int ver =
		iwl_fw_lookup_cmd_ver(mvm->fw, cmd.id, IWL_FW_CMD_VER_UNKNOWN);
				...
				struct ieee80211_sta *ap_sta)
{
	...
	binding_cmd.id_and_color = cpu_to_le32(FW_CMD_ID_AND_COLOR(
		mvmvif->phy_ctxt->id, mvmvif->phy_ctxt->color));
	binding_cmd.action = cpu_to_le32(FW_CTXT_ACTION_ADD);
	binding_cmd.phy = cpu_to_le32(FW_CMD_ID_AND_COLOR(
		mvmvif->phy_ctxt->id, mvmvif->phy_ctxt->color));
	binding_cmd.macs[0] =
	...
	quota->id_and_color = cpu_to_le32(FW_CMD_ID_AND_COLOR(
		mvmvif->phy_ctxt->id, mvmvif->phy_ctxt->color));
...
static int
iwl_mvm_get_wowlan_config(struct iwl_mvm *mvm, struct cfg80211_wowlan *wowlan,
			  ...
			  struct ieee80211_sta *ap_sta)
{
	...
	wowlan_config_cmd->flags = ENABLE_L3_FILTERING | ENABLE_NBNS_FILTERING |
					    ...
					    struct ieee80211_vif *vif)
{
	...
	if (!fw_has_api(&mvm->fw->ucode_capa,
		...
		if (ver == 2) {
			...
			tkip_data.tkip.sta_id = cpu_to_le32(mvmvif->ap_sta_id);
		...
		if (tkip_data.have_tkip_keys) {
			...
			ret = iwl_mvm_send_cmd_pdu(mvm, WOWLAN_TKIP_PARAM,
						   CMD_ASYNC, size,
						   &tkip_data.tkip);
			if (ret)
				return ret;
	...
	if (mvmvif->rekey_data.valid) {
		...
		if (cmd_ver == 4) {
			...
			_kek_kck_cmd = (void *)((u8 *)_kek_kck_cmd +
						sizeof(kek_kck_cmd.sta_id));
		}
...
static int
iwl_mvm_wowlan_config(struct iwl_mvm *mvm, struct cfg80211_wowlan *wowlan,
		      struct iwl_wowlan_config_cmd *wowlan_config_cmd,
		      struct ieee80211_vif *vif, struct iwl_mvm_vif *mvmvif,
		      struct ieee80211_sta *ap_sta)
{
...
static int
iwl_mvm_netdetect_config(struct iwl_mvm *mvm, struct cfg80211_wowlan *wowlan,
			 ...
			 struct ieee80211_vif *vif)
{
	...
	mvm->nd_channels =
		kmemdup(nd_config->channels,
			sizeof(*nd_config->channels) * nd_config->n_channels,
			GFP_KERNEL);
	if (mvm->nd_channels)
...
static int __iwl_mvm_suspend(struct ieee80211_hw *hw,
			     struct cfg80211_wowlan *wowlan, bool test)
{
	struct iwl_mvm *mvm = IWL_MAC80211_GET_MVM(hw);
	...
	if (mvmvif->ap_sta_id == IWL_MVM_INVALID_STA) {
		...
		ret = iwl_mvm_netdetect_config(mvm, wowlan, wowlan->nd_config,
					       vif);
		if (ret)
			goto out;
					  ...
					  struct iwl_wowlan_status_data *status)
{
	...
	if (status->num_of_gtk_rekeys) {
		...
		struct {
			...
			.conf.keyidx = status->gtk.flags &
				       IWL_WOWLAN_GTK_IDX_MASK,
		};
		__be64 replay_ctr;

		IWL_DEBUG_WOWLAN(
			mvm, "Received from FW GTK cipher %d, key index %d\n",
...
out:
	if (iwl_fw_lookup_notif_ver(mvm->fw, LONG_GROUP, WOWLAN_GET_STATUSES,
				    0) < 10) {
...
#define iwl_mvm_parse_wowlan_status_common(_ver)                              \
	static struct iwl_wowlan_status_data                                  \
		*iwl_mvm_parse_wowlan_status_common_##_ver(                   \
			struct iwl_mvm *mvm,                                  \
			struct iwl_wowlan_status_##_ver *data, int len)       \
	{                                                                     \
		struct iwl_wowlan_status_data *status;                        \
		int data_size, i;                                             \
                                                                              \
		...
		status->received_beacons =                                    \
			le32_to_cpu(data->received_beacons);                  \
...
iwl_mvm_parse_wowlan_status_common(v6) iwl_mvm_parse_wowlan_status_common(v7)
	iwl_mvm_parse_wowlan_status_common(v9)
		iwl_mvm_parse_wowlan_status_common(v12)

			static void iwl_mvm_convert_gtk_v2(
				struct iwl_wowlan_status_data *status,
...
#define CHECK(_f)                                                             \
	do {                                                                  \
		...
		BUILD_BUG_ON(                                                 \
			offsetofend(struct iwl_wowlan_gtk_status_v2, _f) !=   \
				 ...
				 struct iwl_wowlan_igtk_status *data)
{
	...
	status->igtk.ipn = ((u64)ipn[5] << 0) | ((u64)ipn[4] << 8) |
			   ((u64)ipn[3] << 16) | ((u64)ipn[2] << 24) |
			   ((u64)ipn[1] << 32) | ((u64)ipn[0] << 40);
}
...
iwl_mvm_send_wowlan_get_status(struct iwl_mvm *mvm, u8 sta_id)
{
	...
	if (!fw_has_api(&mvm->fw->ucode_capa,
		...
		iwl_mvm_convert_key_counters(status,
					     &v7->gtk[0].rsc.all_tsc_rsc);
...
iwl_mvm_get_wakeup_status(struct iwl_mvm *mvm, u8 sta_id)
{
	...
	u32 cmd_size = cmd_ver != IWL_FW_CMD_VER_UNKNOWN ? sizeof(station_id) :
							   0;
					 ...
					 struct ieee80211_vif *vif)
{
	...
	if (mvm->trans->trans_cfg->device_family >= IWL_DEVICE_FAMILY_22000) {
		i = mvm->offload_tid;
		iwl_trans_set_q_ptrs(mvm->trans, mvm_ap_sta->tid_data[i].txq_id,
...
#define ND_QUERY_BUF_LEN \
	(sizeof(struct iwl_scan_offload_profile_match) * IWL_SCAN_MAX_PROFILES)
				    ...
				    int idx)
{
	...
	if (fw_has_api(&mvm->fw->ucode_capa,
		...
		for (i = 0; i < SCAN_OFFLOAD_MATCHING_CHANNELS_LEN * 8; i++)
			if (matches[idx].matching_channels[i / 8] &
			    (BIT(i % 8)))
...
static bool iwl_mvm_rt_status(struct iwl_trans *trans, u32 base, u32 *err_id)
{
	...
	iwl_trans_read_mem_bytes(trans, base, &err_info, sizeof(err_info));
				    ...
				    size_t count, loff_t *ppos)
{
	...
	while (true) {
		...
		if (mvm->d3_test_pme_ptr) {
			pme_asserted = iwl_trans_read_mem32(
				mvm->trans, mvm->d3_test_pme_ptr);
			if (pme_asserted)
				break;
		}
