...
int iwl_mvm_ftm_add_pasn_sta(struct iwl_mvm *mvm, struct ieee80211_vif *vif,
			     u8 *addr, u32 cipher, u8 *tk, u32 tk_len, u8 *hltk,
			     u32 hltk_len)
{
	struct iwl_mvm_ftm_pasn_entry *pasn =
		kzalloc(sizeof(*pasn), GFP_KERNEL);
	u32 expected_tk_len;
...
iwl_mvm_ftm_target_chandef_v1(struct iwl_mvm *mvm,
			      ...
			      u8 *channel, u8 *bandwidth, u8 *ctrl_ch_position)
{
	...
	*ctrl_ch_position = (peer->chandef.width > NL80211_CHAN_WIDTH_20) ?
				    iwl_mvm_get_ctrl_pos(&peer->chandef) :
				    0;
...
iwl_mvm_ftm_target_chandef_v2(struct iwl_mvm *mvm,
			      struct cfg80211_pmsr_request_peer *peer,
			      u8 *channel, u8 *format_bw, u8 *ctrl_ch_position)
{
	u32 freq = peer->chandef.chan->center_freq;
	u8 cmd_ver;
	...
	*channel = ieee80211_frequency_to_channel(freq);
	...
	case NL80211_CHAN_WIDTH_160:
		cmd_ver = iwl_fw_lookup_cmd_ver(
			mvm->fw, WIDE_ID(LOCATION_GROUP, TOF_RANGE_REQ_CMD),
...
static void iter(struct ieee80211_hw *hw, struct ieee80211_vif *vif,
...
static int iwl_mvm_ftm_start_v11(struct iwl_mvm *mvm, struct ieee80211_vif *vif,
				 struct cfg80211_pmsr_request *req)
{
...
static int iwl_mvm_ftm_start_v12(struct iwl_mvm *mvm, struct ieee80211_vif *vif,
				 struct cfg80211_pmsr_request *req)
{
...
static int iwl_mvm_ftm_start_v13(struct iwl_mvm *mvm, struct ieee80211_vif *vif,
				 struct cfg80211_pmsr_request *req)
{
	...
	for (i = 0; i < cmd.num_of_ap; i++) {
		...
		if (peer->ftm.non_trigger_based) {
			target->min_time_between_msr = cpu_to_le16(
				IWL_MVM_FTM_NON_TB_MIN_TIME_BETWEEN_MSR);
			target->burst_period = cpu_to_le16(
...
void iwl_mvm_ftm_abort(struct iwl_mvm *mvm, struct cfg80211_pmsr_request *req)
{
	...
	if (iwl_mvm_send_cmd_pdu(mvm,
				 WIDE_ID(LOCATION_GROUP, TOF_RANGE_ABORT_CMD),
				 0, sizeof(cmd), &cmd))
...
static u64 iwl_mvm_ftm_get_host_time(struct iwl_mvm *mvm, __le32 fw_gp2_ts)
{
	...
	iwl_mvm_get_sync_time(mvm, CLOCK_BOOTTIME, &curr_gp2, &now_from_boot_ns,
			      NULL);
				      ...
				      struct cfg80211_pmsr_result *res)
{
	...
	if (rtt_avg > rtt && (rtt_avg - rtt) > undershoot) {
		...
		IWL_DEBUG_INFO(mvm, "undershoot: val=%lld\n", (rtt_avg - rtt));
	} else if (rtt_avg < rtt && (rtt - rtt_avg) > overshoot) {
		res->ftm.rtt_avg = rtt_avg;
		IWL_DEBUG_INFO(mvm, "overshoot: val=%lld\n", (rtt - rtt_avg));
	}
...
void iwl_mvm_ftm_range_resp(struct iwl_mvm *mvm, struct iwl_rx_cmd_buffer *rxb)
{
	...
	for (i = 0; i < num_of_aps && i < IWL_MVM_TOF_MAX_APS; i++) {
		...
		result.host_time =
			iwl_mvm_ftm_get_host_time(mvm, fw_ap->timestamp);
...
void iwl_mvm_ftm_lc_notif(struct iwl_mvm *mvm, struct iwl_rx_cmd_buffer *rxb)
{
	...
	size_t baselen =
		IEEE80211_MIN_ACTION_SIZE + sizeof(mgmt->u.action.u.ftm);
