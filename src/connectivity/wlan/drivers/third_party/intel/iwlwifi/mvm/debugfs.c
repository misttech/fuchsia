...
static ssize_t iwl_dbgfs_amsdu_len_write(struct ieee80211_sta *sta, char *buf,
					 size_t count, loff_t *ppos)
{
	struct iwl_mvm_sta *mvmsta = iwl_mvm_sta_from_mac80211(sta);
	int i;
...
static int iwl_mvm_coex_dump_mbox(struct iwl_bt_coex_profile_notif *notif,
...
static ssize_t iwl_dbgfs_bt_tx_prio_write(struct iwl_mvm *mvm, char *buf,
					  size_t count, loff_t *ppos)
{
	u32 bt_tx_prio;
...
static ssize_t iwl_dbgfs_bt_force_ant_write(struct iwl_mvm *mvm, char *buf,
					    size_t count, loff_t *ppos)
{
...
static int _iwl_dbgfs_inject_beacon_ie(struct iwl_mvm *mvm, char *bin, int len)
{
	...
	iwl_mvm_mac_ctxt_set_tim(mvm, &beacon_cmd.tim_idx, &beacon_cmd.tim_size,
...
static ssize_t iwl_dbgfs_fw_dbg_collect_write(struct iwl_mvm *mvm, char *buf,
					      size_t count, loff_t *ppos)
{
	if (count == 0)
		return 0;
