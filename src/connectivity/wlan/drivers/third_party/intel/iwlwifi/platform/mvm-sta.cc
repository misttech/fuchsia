...
zx_status_t MvmSta::InstallKey(const fuchsia_wlan_softmac::wire::WlanKeyConfiguration* key_config) {
  ...
  auto cipher_oui = key_config->cipher_oui();
  if (!std::equal(cipher_oui.begin(), cipher_oui.begin() + cipher_oui.size(), kIeeeOui,
    ...
    IWL_ERR(mvmvif, "Cipher OUI must be %02X:%02X:%02X. OUI %02X:%02X:%02X is not supported.",
            kIeeeOui[0], kIeeeOui[1], kIeeeOui[2], cipher_oui[0], cipher_oui[1], cipher_oui[2]);
    return ZX_ERR_NOT_SUPPORTED;
  }
  ...
  if (key_conf->cipher == CIPHER_SUITE_TYPE_TKIP) {
    ...
    //
    ZX_ASSERT_MSG(key_config->key().size() == 32, "TKIP key length must be 32. Found %zu",
                  key_config->key().size());
