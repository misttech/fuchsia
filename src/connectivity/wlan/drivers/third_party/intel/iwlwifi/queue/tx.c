					  ...
					  int num_tbs)
{
	...
	if (trans->trans_cfg->device_family >= IWL_DEVICE_FAMILY_AX210) {
		struct iwl_gen3_bc_tbl_entry *scd_bc_tbl_gen3 =
			txq->bc_tbl.addr;
				      ...
				      struct sk_buff *skb)
{
	 ...
	 */
	if (p->pos + len <
	    (u8 *)page_address(p->page) + page_size - sizeof(void *))
		goto out;
...
__maybe_unused static zx_status_t
iwl_txq_gen2_build_amsdu(struct iwl_trans *trans, struct sk_buff *skb,
			 ...
			 struct iwl_device_tx_cmd *dev_cmd)
{
	...
	while (total_len) {
		...
		*((__be16 *)hdr_page->pos) = cpu_to_be16(length);
		...
		tb_phys = dma_map_single(trans->dev, start_hdr, tb_len,
					 DMA_TO_DEVICE);
		...
		trace_iwlwifi_dev_tx_tb(trans->dev, skb, start_hdr, tb_phys,
					tb_len);
		...
		while (data_left) {
			...
			tb_phys = dma_map_single(trans->dev, tso.data, tb_len,
						 DMA_TO_DEVICE);
...
static struct iwl_tfh_tfd *iwl_txq_gen2_build_tx_amsdu(
	struct iwl_trans *trans, struct iwl_txq *txq,
	struct iwl_device_tx_cmd *dev_cmd, struct ieee80211_mac_packet *pkt,
	struct iwl_cmd_meta *out_meta, int hdr_len, int tx_cmd_len)
{
#if 0 // NEEDS_PORTING
	int idx = iwl_txq_get_cmd_index(txq, txq->write_ptr);
...
__maybe_unused static zx_status_t
iwl_txq_gen2_tx_add_frags(struct iwl_trans *trans, struct sk_buff *skb,
			  struct iwl_tfh_tfd *tfd,
...
static struct iwl_tfh_tfd *iwl_txq_gen2_build_tx(
	struct iwl_trans *trans, struct iwl_txq *txq,
	struct iwl_device_tx_cmd *dev_cmd, struct ieee80211_mac_packet *pkt,
	struct iwl_cmd_meta *out_meta, int hdr_len, int tx_cmd_len, bool pad)
{
	int idx = iwl_txq_get_cmd_index(txq, txq->write_ptr);
	struct iwl_tfh_tfd *tfd = iwl_txq_get_tfd(trans, txq, idx);
	dma_addr_t tb_phys;
	...
	/* map the data for TB1 */
	struct iwl_device_tx_cmd *out_cmd =
		iwl_iobuf_virtual(txq->entries[idx].cmd);
	memcpy(out_cmd, ((void *)dev_cmd) + IWL_FIRST_TB_SIZE, tb1_len);
	iwl_iobuf_cache_flush(txq->entries[idx].cmd, 0, tb1_len);
	tb_phys = iwl_iobuf_physical(txq->entries[idx].cmd);
	/*
	 * No need for _with_wa(), we ensure (via alignment) that the data
	 * here can never cross or end at a page boundary.
	 */
	...
	tb2_len = (int)(pkt->header_size + pkt->body_size - hdr_len);

#if 1 // NEEDS_PORTING
	if (tb2_len > 0) {
		// Copy the packet body to the 'dup_io_buf'.
		struct iwl_iobuf *dup_io_buf = txq->entries[idx].dup_io_buf;
		ZX_ASSERT(dup_io_buf == NULL);

		iwl_iobuf_allocate_contiguous(trans->dev, tb2_len, &dup_io_buf);
		void *virt_addr = iwl_iobuf_virtual(dup_io_buf);
		memcpy(virt_addr, pkt->body, tb2_len);
		tb_phys = iwl_iobuf_physical(dup_io_buf);
		iwl_iobuf_cache_flush(dup_io_buf, 0, tb2_len);
		txq->entries[idx].dup_io_buf = dup_io_buf;

		zx_status_t ret = iwl_txq_gen2_set_tb_with_wa(
			trans, pkt, tfd, tb_phys, pkt->body, (u16)tb2_len,
			NULL);
		if (ret != ZX_OK) {
			goto out_err;
		}
...
static struct iwl_tfh_tfd *
iwl_txq_gen2_build_tfd(struct iwl_trans *trans, struct iwl_txq *txq,
		       struct iwl_device_tx_cmd *dev_cmd,
		       struct ieee80211_mac_packet *pkt,
		       struct iwl_cmd_meta *out_meta)
{
	...
	amsdu = ieee80211_is_data_qos(fh) &&
		(ieee80211_get_qos_ctl(fh) & IEEE80211_QOS_CTRL_A_MSDU_PRESENT);
	 ...
	 */
	if (false /* NEEDS_PORTING amsdu && skb_shinfo(skb)->gso_size */)
...
int iwl_txq_space(struct iwl_trans *trans, const struct iwl_txq *q)
{
	 ...
	 */
	if (q->n_window <
	    (int)(trans->trans_cfg->base_params->max_tfd_queue_size))
		max = q->n_window;
	else
			    ...
			    struct iwl_device_tx_cmd *dev_cmd, int txq_id)
{
	...
	if (iwl_txq_space(trans, txq) < txq->high_mark) {
		...
		if (available_space < TX_RESERVED_SPACE) {
			...
			IWL_WARN(
				trans,
				"dropped packet due to small available space: %zu\n",
				available_space);
			return ZX_OK;
		}
	}
	...
	zx_status_t status = iwl_iobuf_allocate_contiguous(
		trans->dev, TFD_MAX_PAYLOAD_SIZE, &txq->entries[idx].cmd);
	if (status != ZX_OK) {
	...
	dev_cmd->hdr.sequence =
		cpu_to_le16((u16)(QUEUE_TO_SEQ(txq_id) | INDEX_TO_SEQ(idx)));
...
static zx_status_t iwl_queue_init(struct iwl_txq *q, int slots_num)
{
	...
	if (!is_power_of_2(slots_num)) {
		IWL_WARN(q,
			 "The slots num must be the power of 2, %d instead.\n",
			 slots_num);
		return ZX_ERR_INVALID_ARGS;
	}
...
zx_status_t iwl_txq_init(struct iwl_trans *trans, struct iwl_txq *txq,
			 int slots_num, bool cmd_queue)
{
	...
	if (tfd_queue_max_size & (tfd_queue_max_size - 1)) {
		IWL_WARN(
			trans,
			"Max tfd queue size must be a power of two, but is %u\n",
			tfd_queue_max_size);
		return ZX_ERR_INVALID_ARGS;
	}
...
void iwl_txq_log_scd_error(struct iwl_trans *trans, struct iwl_txq *txq)
{
	...
	IWL_ERR(trans,
		...
		jiffies_to_msecs(txq->wd_timeout), txq->read_ptr,
		txq->write_ptr,
...
zx_status_t iwl_txq_alloc(struct iwl_trans *trans, struct iwl_txq *txq,
			  int slots_num, bool cmd_queue)
{
	...
	iwl_irq_timer_create(trans->dev, iwl_txq_stuck_timer, txq,
			     &txq->stuck_timer);
	txq->trans = trans;
	...
	if (cmd_queue)
		for (int i = 0; i < slots_num; i++) {
			zx_status_t status = iwl_iobuf_allocate_contiguous(
				trans->dev, sizeof(struct iwl_device_cmd),
				&txq->entries[i].cmd);
			if (status != ZX_OK) {
				goto error;
			}
	 ...
	 * shared with device */
	zx_status_t status =
		iwl_iobuf_allocate_contiguous(trans->dev, tfd_sz, &txq->tfds);
	if (status != ZX_OK) {
		goto error;
	}
	txq->dma_addr = iwl_iobuf_physical(txq->tfds);

	BUILD_BUG_ON(sizeof(struct iwl_pcie_first_tb_buf) !=
		     IWL_FIRST_TB_SIZE_ALIGN);

	tb0_buf_sz = sizeof(struct iwl_pcie_first_tb_buf) * slots_num;

	status = iwl_iobuf_allocate_contiguous(trans->dev, tb0_buf_sz,
					       &txq->first_tb_bufs);
	if (status != ZX_OK) {
		goto err_free_tfds;
	}
...
static zx_status_t iwl_txq_dyn_alloc_dma(struct iwl_trans *trans, int size,
					 ...
					 struct iwl_txq **out_txq)
{
	...
	// Note that the ownership will be transferred to trans->txqs.txq[] in iwl_txq_alloc_response().
...
static zx_status_t iwl_txq_alloc_response(struct iwl_trans *trans,
					  struct iwl_txq *txq,
...
zx_status_t iwl_txq_dyn_alloc(struct iwl_trans *trans, u32 flags, u32 sta_mask,
			      u8 tid, int size, zx_duration_t timeout,
			      size_t *out_qid)
{
	struct iwl_txq *txq;
	union {
	...
	// 'txq' will be allocated in iwl_txq_dyn_alloc_dma(), and the ownership will be transfered to
	// trans->txqs.txq[] inside iwl_txq_alloc_response().
...
zx_status_t iwl_txq_gen2_init(struct iwl_trans *trans, int txq_id,
			      int queue_size)
{
...
// we update the TFD entry only (zero-ing num_tbs).
//
void iwl_txq_gen1_tfd_unmap(struct iwl_trans *trans, struct iwl_cmd_meta *meta,
				      ...
				      int num_tbs)
{
	...
	__le16 bc_ent;
	struct iwl_device_tx_cmd *dev_cmd =
		iwl_iobuf_virtual(txq->entries[txq->write_ptr].cmd);
				     ...
				     struct iwl_txq *txq)
{
	...
	__le16 bc_ent;
	struct iwl_device_tx_cmd *dev_cmd =
		iwl_iobuf_virtual(txq->entries[read_ptr].cmd);
	struct iwl_tx_cmd *tx_cmd = (void *)dev_cmd->payload;
						...
						struct iwl_host_cmd *cmd)
{
	...
	ret = sync_completion_wait(&trans->wait_command_queue,
