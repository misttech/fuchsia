...
static void FakeEchoWrite32(struct iwl_trans* trans, uint32_t ofs, uint32_t val) {
  ...
  ASSERT_EQ(ZX_OK, iwl_iobuf_allocate_contiguous(trans->dev, 128, &io_buf));
...
class PcieTest : public testing::Test {
 public:
  PcieTest() {
    ...
    EXPECT_EQ(ZX_OK, task_loop_->StartThread("iwlwifi-test-task-worker", nullptr));
    ...
    // Below variable shall be set in iwl_pci_probe(), but the unittest code doesn't go thru it.
    trans_pcie_->num_rx_bufs = RX_QUEUE_SIZE;  // for mq_rx_supported == false
...
TEST_F(PcieTest, RxInit) {
  ...
  ASSERT_EQ(ZX_OK, iwl_pcie_rx_init(trans_));

  EXPECT_EQ(trans_pcie_->rxq->read, 0);
  EXPECT_EQ(trans_pcie_->rxq->write, 0);
  EXPECT_EQ(trans_pcie_->rxq->free_count, RX_POOL_SIZE(trans_pcie_->num_rx_bufs));
  EXPECT_EQ(trans_pcie_->rxq->write_actual, 0);
  EXPECT_EQ(trans_pcie_->rxq->queue_size, RX_QUEUE_SIZE);
  EXPECT_GE(list_length(&trans_pcie_->rxq->rx_free), RX_POOL_SIZE(trans_pcie_->num_rx_bufs));
  EXPECT_NE(nullptr, trans_pcie_->rxq->rb_status);

  struct iwl_rx_mem_buffer* rxb;
  list_for_every_entry (&trans_pcie_->rxq->rx_free, rxb, struct iwl_rx_mem_buffer, list) {
    EXPECT_NE(nullptr, rxb->io_buf);
...
TEST_F(PcieTest, MqRxInit) {
  ...
  trans_->cfg = &iwl9260_2ac_cfg;     // Also overwrite the chip configuration with 9260, which uses
                                      // multiple Rx queues.
...
TEST_F(PcieTest, IctTable) {
  ASSERT_EQ(ZX_OK, iwl_pcie_alloc_ict(trans_));
...
TEST_F(PcieTest, RxInterrupts) {
  ...
  ASSERT_EQ(ZX_OK, iwl_pcie_tx_init(trans_));
  ...
  ASSERT_EQ(ZX_OK, iwl_pcie_isr(trans_));
...
TEST_F(TxTest, AsyncHostCommandEmpty) {
  ...
  ASSERT_EQ(ZX_OK, iwl_trans_pcie_send_hcmd(trans_, &hcmd));
}
...
TEST_F(TxTest, AsyncHostCommandOneFragment) {
  ...
  EXPECT_EQ(0, std::memcmp(out_cmd->payload, fragment, sizeof(fragment)));
}
...
TEST_F(TxTest, AsyncHostCommandTwoFragments) {
  ...
  struct iwl_device_cmd* out_cmd =
      static_cast<iwl_device_cmd*>(iwl_iobuf_virtual(txq->entries[cmd_idx].cmd));
  EXPECT_EQ(0, std::memcmp(out_cmd->payload, fragment1, sizeof(fragment1)));
  EXPECT_EQ(0, std::memcmp(out_cmd->payload + sizeof(fragment1), fragment2, sizeof(fragment2)));
}
...
TEST_F(TxTest, SyncTwoFragmentsWithOneDup) {
  ...
  struct iwl_device_cmd* out_cmd =
      static_cast<iwl_device_cmd*>(iwl_iobuf_virtual(txq->entries[cmd_idx].cmd));
  EXPECT_EQ(0, std::memcmp(out_cmd->payload, fragment0, sizeof(fragment0)));
  EXPECT_EQ(0, std::memcmp(out_cmd->payload + sizeof(fragment0), fragment1, sizeof(fragment1)));
  ...
  EXPECT_EQ(0, std::memcmp(dup_buf, fragment1, sizeof(fragment1)));
}
