...
BASE_READ_WRITE_ENCRYPTED_CHARACTERISTIC_UUID = (
    "0000FFF9-0000-1000-8000-00805F9B34FB"
)
BASE_READ_WRITE_ENCRYPTED_MITM_CHARACTERISTIC_UUID = (
...
class GATTProxy(ProfileProxy):
    ...
    def MMI_IUT_INITIATE_CONNECTION(self, test, pts_addr: bytes, **kwargs):
        ...
        # TODO(https://fxbug.dev/450959687): Do we need to comment this back in?
    ...
    def MMI_IUT_SEND_PREPARE_WRITE_REQUEST_VALID_SIZE(
        ...
        assert self.connection is not None
        matches = re.findall(
            "'([a0-Z9]*)'O and size = '([a0-Z9]*)'", description
        )
        handle = int(matches[0][0], 16)
        data = bytes([1]) * int(matches[0][1])
        self.gatt.WriteAttFromHandle(
            connection=self.connection, handle=handle, value=data
        )
        return "OK"
    ...
    def MMI_SEND_PRIMARY_SERVICE_UUID(self, description: str, **kwargs):
        ...
        self.services = self.gatt.DiscoverServiceByUuid(
            connection=self.connection, uuid=uuid
        ).services
        return "OK"
    ...
    def MMI_CONFIRM_PRIMARY_SERVICE(self, test, description: str, **kwargs):
        ...
        if test not in MMI_SERVER:
            ...
            all_matches = list(
                map(formatUuid, re.findall("'([a0-Z9]*)'O", description))
            )
            assert all(
                uuid in list(map(lambda service: service.uuid, self.services))
    ...
    def MMI_IUT_DISCOVER_SERVICE_UUID(self, description: str, **kwargs):
        ...
        self.characteristics = getCharacteristicsForServiceUuid(
            self.services, service_uuid
        )
        return "OK"

    def MMI_CONFIRM_ALL_CHARACTERISTICS_SERVICE(
        self, description: str, **kwargs
    ):
        """
        ...
        all_matches = list(
            map(stringCharHandleToInt, re.findall("'([a0-Z9]*)'O", description))
        )
        assert all(
            handle in list(map(lambda char: char.handle, self.characteristics))
    ...
    def MMI_IUT_DISCOVER_SERVICE_UUID_RANGE(self, description: str, **kwargs):
        ...
        self.characteristics = getCharacteristicsRange(
            self.services,
            stringHandleToInt(handles[0]),
            stringHandleToInt(handles[1]),
            uuid,
        )
        return "OK"
    ...
    def MMI_CONFIRM_CHARACTERISTICS(self, description: str, **kwargs):
        ...
        for characteristic in self.characteristics:
            if characteristic.handle == stringHandleToInt(
                all_matches[0]
    ...
    def MMI_IUT_DISCOVER_DESCRIPTOR_RANGE(self, description: str, **kwargs):
        ...
        self.descriptors = getDescriptorsRange(
            self.services,
            stringHandleToInt(handles[0]),
            stringHandleToInt(handles[1]),
        )
    ...
    def MMI_CONFIRM_CHARACTERISTICS_DESCRIPTORS(
        self, description: str, **kwargs
    ):
        """
    ...
    def MMI_IUT_DISCOVER_ALL_SERVICE_RECORD(
        self, pts_addr: bytes, description: str, **kwargs
    ):
        """
        ...
        self.services = self.gatt.DiscoverServicesSdp(
            address=pts_addr
        ).service_uuids
        assert uuid in self.services
        return "Yes"

    def MMI_IUT_SEND_READ_CHARACTERISTIC_HANDLE(
        self, description: str, **kwargs
    ):
        """
        ...
        def read():
            ...
            self.read_response = self.gatt.ReadCharacteristic(
                connection=self.connection,
    ...
    def MMI_IUT_CONFIRM_READ_INVALID_HANDLE(self, **kwargs):
        ...
        if type(self.read_response) is ReadCharacteristicResponse:
            ...
            assert INVALID_HANDLE in list(
                map(
                    lambda characteristic_read: characteristic_read.status,
    ...
    def MMI_IUT_CONFIRM_READ_NOT_PERMITTED(self, **kwargs):
        ...
        if type(self.read_response) is ReadCharacteristicResponse:
            assert (
                self.read_response.status == READ_NOT_PERMITTED
                or self.read_response.status == UNKNOWN_ERROR
            )
        elif type(self.read_response) is ReadCharacteristicsFromUuidResponse:
            assert self.read_response.characteristics_read is not None
            status_list = list(
            ...
            assert (
                READ_NOT_PERMITTED in status_list
                or UNKNOWN_ERROR in status_list
            )
        return "Yes"
    ...
    def MMI_IUT_CONFIRM_READ_AUTHENTICATION(self, **kwargs):
        ...
        if type(self.read_response) is ReadCharacteristicResponse:
            ...
            assert self.read_response.characteristics_read is not None
            assert INSUFFICIENT_AUTHENTICATION in list(
                map(
                    lambda characteristic_read: characteristic_read.status,
                    self.read_response.characteristics_read,
                )
    ...
    def MMI_IUT_CONFIRM_ATTRIBUTE_NOT_FOUND(self, **kwargs):
        ...
        if type(self.read_response) is ReadCharacteristicResponse:
            assert (
                self.read_response.status == ATTRIBUTE_NOT_FOUND
                or self.read_response.status == UNKNOWN_ERROR
            )
        elif type(self.read_response) is ReadCharacteristicsFromUuidResponse:
            assert self.read_response.characteristics_read is not None
            ...
            assert (
                ATTRIBUTE_NOT_FOUND in status_list
                or UNKNOWN_ERROR in status_list
            )
        return "Yes"
    ...
    def MMI_IUT_CONFIRM_READ_APPLICATION(self, **kwargs):
        ...
        if type(self.read_response) is ReadCharacteristicResponse:
            ...
            assert self.read_response.characteristics_read is not None
            assert APPLICATION_ERROR in list(
                map(
                    lambda characteristic_read: characteristic_read.status,
                    self.read_response.characteristics_read,
                )
    ...
    def MMI_IUT_CONFIRM_READ_CHARACTERISTIC_VALUE(
        self, description: str, **kwargs
    ):
        """
        ...
        characteristic_value = bytes.fromhex(
            re.findall("'([a0-Z9]*)'O", description)[0]
        )
        if type(self.read_response) is ReadCharacteristicResponse:
            ...
            assert self.read_response.characteristics_read is not None
            assert characteristic_value in list(
                map(
    ...
    def MMI_IUT_SEND_WRITE_REQUEST(self, description: str, **kwargs):
        ...
        def write():
            nonlocal handle
            nonlocal data
            self.write_response = self.gatt.WriteAttFromHandle(
                connection=self.connection, handle=handle, value=data
            )
    ...
    def _mmi_150(self, description: str, **kwargs):
        ...
        return self.MMI_IUT_WRITE_SUPPORT_FEATURE_MULTIPLE_HANDLE_VALUE(
            description, **kwargs
        )

    def MMI_IUT_WRITE_SUPPORT_FEATURE_MULTIPLE_HANDLE_VALUE(
        self, description: str, **kwargs
    ):
        """
        Please send an ATT_Write_Request to Client Support Features handle =
    ...
    def MMI_IUT_SEND_PREPARE_WRITE_GREATER_OFFSET(
        self, description: str, **kwargs
    ):
        """
        ...
        assert self.connection is not None
        matches = re.findall(
            "'([a0-Z9]*)'O and offset greater than '([a0-Z9]*)'", description
        )
        handle = stringHandleToInt(matches[0][0])
    ...
    def MMI_IUT_SEND_WRITE_REQUEST_GREATER(self, description: str, **kwargs):
        ...
        assert self.connection is not None
        matches = re.findall(
            "'([a0-Z9]*)'O with greater than '([a0-Z9]*)'", description
        )
        handle = stringHandleToInt(matches[0][0])
    ...
    def MMI_IUT_SEND_PREPARE_WRITE_REQUEST_GREATER(
        self, description: str, **kwargs
    ):
        """
    ...
    def MMI_IUT_ENTER_HANDLE_READ_NOT_PERMITTED(self, **kwargs):
        ...
        return "{:04x}".format(
            self.last_added_service.service.characteristics[0].handle
        )
    ...
    def MMI_IUT_SEND_INDICATION(self, description: str, **kwargs):
        ...
        self.gatt.SetCharacteristicNotificationFromHandle(
            connection=self.connection,
            handle=handle,
    ...
    def MMI_IUT_RECEIVE_INDICATION(self, **kwargs):
        ...
        for handle in self.handles:
            self.characteristic_notification_received = (
                self.gatt.WaitCharacteristicNotification(
                    connection=self.connection, handle=handle
                ).characteristic_notification_received
            )
    ...
    def MMI_IUT_SEND_NOTIFICATION(self, description: str, **kwargs):
        ...
        self.gatt.SetCharacteristicNotificationFromHandle(
            connection=self.connection,
            handle=handle,
            enable_value=ENABLE_NOTIFICATION_VALUE,
        )
...
def compareIncludedServices(
    service, service_handle, included_handle, included_uuid
):
    """
    ...
    for included_service in service.included_services:
        if (
            service.handle == (service_handle - included_service_count)
...
def getCharacteristicsRange(services, start_handle, end_handle, uuid):
    ...
    for service in services:
        for characteristic in service.characteristics:
            if (
                characteristic.handle >= start_handle
    ...
    for service in services:
        for characteristic in service.characteristics:
            for descriptor in characteristic.descriptors:
                if (
                    descriptor.handle >= start_handle
                    and descriptor.handle <= end_handle
                ):
