...
class RootCanal:
    ...
    def select_pts_dongle(self, dongle: Dongle):
        ...
        print("Sending dongle command...")
        self.channel.send_command(
            "set_device_configuration", [pts_id, dongle.value]
        )
        print("Dongle command sent")
        ...
        for device_id, _ in devices:
            for phy_id, _, phy_devices in phys:
                if device_id in phy_devices:
                    self.channel.send_command(
                        "del_device_from_phy", [device_id, phy_id]
                    )
        ...
        for device_id, device_name in devices:
            ...
            for phy_id, phy_name, phy_devices in phys:
                if phy_name in target_phys and not device_id in phy_devices:
                    self.channel.send_command(
                        "add_device_to_phy", [device_id, phy_id]
                    )
    ...
    def _read_device_list(self):
        ...
        for line in response.split("\n"):
            ...
            if line.startswith("Devices") or line.startswith("Phys"):
                ...
                phy_name = parts[1]
                phy_devices = [
                    int(id.strip()) for id in parts[2].split(",") if id.strip()
                ]
