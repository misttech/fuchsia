...
impl RemotePeer {
    ...
    /// Immediately resolves to Ok(Err(ErrorCode::InProgress)) if the connection type is already connecting.
    pub fn connect(
        &mut self,
    ...
    > {
        ...
        async move {
            let Some(mut params) = connect_parameters else {
                return Ok(Err(fidl_fuchsia_bluetooth::ErrorCode::InProgress));
            };
