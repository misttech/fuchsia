...
bool AttachTokenSucceedsV1(
  ...
  auto allocator_2 = connect_to_sysmem_service_v1();
  ...
  auto collection_client_3_set_constraints = [&allocator_2, &token_client_3, &constraints_3,
    ...
    if (collection_3.is_valid()) {
      ...
      auto [tmp_attached_client, tmp_attached_server] =
          fidl::Endpoints<fuchsia_sysmem::BufferCollectionToken>::Create();
      auto attach_token_status =
          collection_3->AttachToken(ZX_RIGHT_SAME_RIGHTS, std::move(tmp_attached_server));
      EXPECT_OK(attach_token_status);
      IF_FAILURES_RETURN();
...
TEST(Sysmem, NoSyncV1) {
  auto allocator_1 = connect_to_sysmem_service_v1();
