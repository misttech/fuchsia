...
func (f *FFXTool) TargetGetLastRebootReason(ctx context.Context, target string) (string, error) {
	reason, err := f.targetGetLastRebootReason(ctx, target)
	if err != nil {
		return reason, err
	}

	// TODO(b/496577711): This is a workaround for emulator targets that report "Cold" as the last
	// reboot reason after an actually successful OTA. If this happens, we find the real reboot
	// reason by reading the graceful shutdown info file. Once the emulator and the physical devices
	// report the same reboot reason, this workaround can be removed.
	if reason == "Cold" {
		shutdownReason, err := f.targetGetGracefulShutdownInfo(ctx, target)
		if err == nil && shutdownReason == "SYSTEM UPDATE" {
...
func (f *FFXTool) targetGetGracefulShutdownInfo(ctx context.Context, target string) (string, error) {
	args := []string{
		"--direct",
		"--target", target,
		"target", "ssh", "cat",
		// This corresponds to the /core/feedback moniker, see
		// //src/sys/core/core_component_id_index.json5
		"tmp/eb345fb7dcaa4260ee0c65bb73ef0ec5341b15a4f603f358d6631c4be6bf7080/graceful_shutdown_info.json",
	}

	stdout, err := f.runFFXCmd(ctx, args...)
	if err != nil {
		return "", fmt.Errorf("reading graceful_shutdown_info.json failed: %w", err)
	}
	...
	if len(showInfo.Reasons) > 0 {
		return showInfo.Reasons[0], nil
	}

	return "", fmt.Errorf("no reboot reason found in graceful_shutdown_info.json")
}
