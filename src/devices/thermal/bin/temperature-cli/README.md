...
```
temperature-cli [device] <command> [args...]
...
- For ADC commands, it will scan for ADC devices. For other commands, it will scan for temperature devices.
...
*   **`wait`**: Wait for any trippoint to be triggered.
    ...
    ```
*   **`trip`** (Debug): Manually trip a trippoint (requires Debug service).
    ```bash
    $ temperature-cli /svc/fuchsia.hardware.trippoint.DebugService/default/debug trip 1
    ```
