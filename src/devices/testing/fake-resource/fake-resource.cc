...
class Resource final : public fake_object::Object {
 public:
  ...
  static zx_status_t Create(zx_paddr_t base, size_t size, zx_rsrc_kind_t kind,
    ...
    *out = std::make_shared<Resource>(base, size, kind, flags, name, name_len, is_root);
    return ZX_OK;
  }
...
zx_status_t zx_resource_create(zx_handle_t parent_rsrc, uint32_t options, uint64_t base,
  ...
  ZX_ASSERT(Resource::Create(base, size, kind, flags, name, name_size, /*is_root=*/false,
                             &new_res) == ZX_OK);
...
zx_status_t fake_resource_create(zx_rsrc_kind_t kind, zx_handle_t* out) {
  ...
  std::shared_ptr<fake_object::Object> new_res;
  ZX_ASSERT(Resource::Create(0, 0, kind, 0, name.data(), name.size(), /*is_root=*/true, &new_res) ==
            ZX_OK);
  zx::result add_res = fake_object::FakeHandleTable().Add(std::move(new_res));
  if (add_res.is_ok()) {
    *out = add_res.value();
