...
zx::result<> TpmLpc::PerformTransfer(uint16_t address, fidl::VectorView<uint8_t>& buf,
                                     bool do_write) {
  if (address > mmio_.get_size() || buf.size() > mmio_.get_size()) {
    zxlogf(ERROR, "Cannot proceed address is out of range.");
    return zx::error(ZX_ERR_INVALID_ARGS);
  }
  if (buf.size() + address > mmio_.get_size()) {
  ...
  if (do_write) {
    size_t aligned_size = (buf.size() / kAddressAlignment) * kAddressAlignment;
    ...
    size_t remainder = buf.size() % kAddressAlignment;
    if (remainder != 0) {
      size_t offset = buf.size() - remainder;
