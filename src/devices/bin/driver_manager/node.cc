...
void Node::CompleteBind(zx::result<> result, bool silent) {
  ...
  return std::make_shared<BindResultTracker>(
      1, [weak_self = weak_from_this(),
        ...
        if (info.size() < 1) {
          ...
          self->CompleteBind(zx::error(ZX_ERR_NOT_FOUND), silent);
        } else if (info.size() > 1) {
          fdf_log::error("Unexpectedly bound multiple drivers to a single node");
          self->OnMatchError(ZX_ERR_BAD_STATE);
          self->CompleteBind(zx::error(ZX_ERR_BAD_STATE), silent);
        }
      });
}
