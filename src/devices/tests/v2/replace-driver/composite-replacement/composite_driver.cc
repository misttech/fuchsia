...
class CompositeDriver : public fdf::DriverBase2 {
 public:
  CompositeDriver() : fdf::DriverBase2("composite_replaced") {}

  zx::result<> Start(fdf::DriverContext context) override {
    auto incoming_ptr = std::shared_ptr<fdf::Namespace>(context.take_incoming());
    node_client_.Bind(take_node());

    zx::result result = helpers::AddChild(logger(), "J_replaced", node_client_,
...
FUCHSIA_DRIVER_EXPORT2(CompositeDriver);
