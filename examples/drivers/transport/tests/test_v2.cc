...
TEST(DriverTransportTest, ParentChildExists) {
...

  {
    ...
    auto node = driver_test_realm::WaitForNode(realm, "dev.sys.test.transport-parent");
...

  {
    ...
    auto node =
        driver_test_realm::WaitForNode(realm, "dev.sys.test.transport-parent.transport-child");
    ASSERT_TRUE(node.is_ok());
  }
