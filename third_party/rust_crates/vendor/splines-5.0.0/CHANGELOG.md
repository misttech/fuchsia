...
- [15ac1ce](https://git.sr.ht/~hadronized/splines/commit/15ac1ce) Bump glam dependency upper limit to <0.31.
- [4de9a0f](https://git.sr.ht/~hadronized/splines/commit/4de9a0f) Remove impl-* and serialization features.
- [4b41aab](https://git.sr.ht/~hadronized/splines/commit/4b41aab) Migrate to Rust 2024.

# 4.4.2

> Sun Jan 26 2025

- [9143cfd](https://git.sr.ht/~hadronized/splines/commit/9143cfd) Fix `no_std+num-traits` conditional compilation.
  This was not compiling properly, because of `cos` not being in `core` (it’s actually in `std`). If you
  want to use `splines` in a `no_std` environment, it’s recommended to disable all features and enable the
  `num-traits` feature if you want to use `Interpolate` default implementors.

# 4.4.1

> Mon Dec 23 2024

- [2383836](https://git.sr.ht/~hadronized/splines/commit/2383836) Update documentation for Catmull-Rom.

# 4.4.0

> Oct 29, 2024

- fix: f64 glam interpolation support [d9a5b30](https://git.sr.ht/~hadronized/splines/commit/d9a5b30)
...
- Switch from holding keys in `Vec` into `VecDeque`. [1b4fe44](https://git.sr.ht/~hadronized/splines/commit/1b4fe44)

# 4.3.1

> Nov 22, 2023

- Add `Default` implementation for `Spline`. [c6ba847](https://git.sr.ht/~hadronized/splines/commit/c6ba847)

# 4.3

> Sep 23, 2023

- Add support for `glam-0.23` and `glam-0.24`. [cdc48a4](https://git.sr.ht/~hadronized/splines/commit/cdc48a4)
- Add `Spline::clear` to clear a spline keys without deallocating its internal storage. [eca09f1](https://git.sr.ht/~hadronized/splines/commit/eca09f1)
