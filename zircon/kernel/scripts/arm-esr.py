...
def unpack_instruction_abort_iss(iss: int) -> tuple[int, int, int, int, int]:
    ifsc = iss & ((1 << 6) - 1)
    iss >>= 6  # eat IFSC
...
def decode_instruction_abort(ec: int, ec_desc: str, iss: int) -> None:
...
# Decode a data abort.
def decode_data_abort(ec: int, ec_desc: str, iss: int) -> None:
    (
        dfsc,
        wnr,
        s1ptw,
...
# Decode just the EC.
def decode_ec(ec_desc: str) -> None:
    print("EC:", ec_desc)


# Decode the EC and ISS.
def decode_ec_iss(ec: int, iss: int) -> None:
    if ec == 0b000000:
        ...
        decode_ec("Branch target exception")
    elif ec == 0b001110:
        ...
        decode_ec(
            "Trapped MSRR, MRRS, or system instruction in AArch64 state, that is not reported using EC 0b000000"
        )
    elif ec == 0b010101:
        ...
        decode_ec("Exception from an access to a TSTART instruction at EL0")
    elif ec == 0b011100:
        decode_ec(
            "Exception from a Pointer Authentication instruction authentication failure"
        )
    elif ec == 0b011101:
        decode_ec("Access to SME functionality trapped instruction")
    elif ec == 0b011111:
        ...
        decode_ec("Memory operation exception")
    elif ec == 0b101000:
        ...
        decode_ec("GCS exception")
    elif ec == 0b101111:
        decode_ec("exception interrupt")
    elif ec == 0b110000:
        decode_ec(
        ...
        decode_ec("Profiling exception")
    else:
        decode_ec("Unexpected EC value")


def decode_esr(esr: int) -> None:
