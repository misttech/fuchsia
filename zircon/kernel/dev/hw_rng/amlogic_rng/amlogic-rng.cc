...
bool rng_is_ready() { return (MmioRead32(rng_status) & AML_RNG_READY) == 1; }
...
void AmlogicRngInit(const zbi_dcfg_amlogic_rng_driver_t& config) {
  ...
  rng_data =
      reinterpret_cast<MMIO_PTR volatile uint32_t*>(periph_paddr_to_vaddr(config.rng_data_phys));
  rng_status =
      reinterpret_cast<MMIO_PTR volatile uint32_t*>(periph_paddr_to_vaddr(config.rng_status_phys));
