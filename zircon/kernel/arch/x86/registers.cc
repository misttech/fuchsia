...
void x86_read_debug_status(uint64_t* dr6) {
  ...
  uint64_t value = 0;
  x86_read_raw_debug_status(&value);
...
void x86_write_debug_status(uint64_t dr6) {
  ...
  __asm__ volatile("mov %0, %%dr6" ::"r"(full_value));
}
...
void x86_read_hw_debug_regs(x86_debug_state_t* debug_state) {
  __asm__ volatile("mov %%dr0, %0" : "=r"(debug_state->dr[0]));
  __asm__ volatile("mov %%dr1, %0" : "=r"(debug_state->dr[1]));
  __asm__ volatile("mov %%dr2, %0" : "=r"(debug_state->dr[2]));
  __asm__ volatile("mov %%dr3, %0" : "=r"(debug_state->dr[3]));

  x86_read_debug_status(&debug_state->dr6);

  __asm__ volatile("mov %%dr7, %0" : "=r"(debug_state->dr7));
}

void x86_write_hw_debug_regs(const x86_debug_state_t* debug_state) {
  __asm__ volatile("mov %0, %%dr0" ::"r"(debug_state->dr[0]));
  __asm__ volatile("mov %0, %%dr1" ::"r"(debug_state->dr[1]));
  __asm__ volatile("mov %0, %%dr2" ::"r"(debug_state->dr[2]));
  __asm__ volatile("mov %0, %%dr3" ::"r"(debug_state->dr[3]));
  ...
  __asm__ volatile("mov %0, %%dr7" ::"r"(debug_state->dr7));
}
