...
class ThreadDispatcher final : public SoloDispatcher<ThreadDispatcher, ZX_DEFAULT_THREAD_RIGHTS>,
 ...
 public:
  ...
  TaskRuntimeStats GetCompensatedTaskRuntimeStats(zx_time_t now) const;
  ...
  // Update the runtime stats for this thread. This is called by Scheduler to update the runtime
  // stats of the thread as it changes thread state. Must be called with interrupts disabled.
  void UpdateRuntimeStats(const ThreadRuntimeStats::ThreadStats& delta, bool update_process_stats);
