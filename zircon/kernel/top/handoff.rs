...
unsafe extern "C" {
    ...
    fn cpp_physhandoff_get_acpi_rsdp(acpi_rsdp: *mut u64) -> bool;
}
...
/// Returns the physical address of the ACPI RSDP, if available in the phys handoff.
pub fn acpi_rsdp() -> Option<u64> {
    let mut phys = 0u64;
    // SAFETY: `cpp_physhandoff_get_acpi_rsdp` writes to `acpi_rsdp` if valid handoff is present.
    let has_acpi_rsdp = unsafe { cpp_physhandoff_get_acpi_rsdp(&mut phys) };
    if has_acpi_rsdp { Some(phys) } else { None }
}
