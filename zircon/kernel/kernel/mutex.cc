 ...
 public:
  ...
  void KernelMutexUncontestedAcquire(const void* mutex_id) {
    if constexpr (Level == KernelMutexTracingLevel::All) {
      KernelMutexTrace(TAG_KERNEL_MUTEX_ACQUIRE, mutex_id, nullptr, 0);
    }
  }

  void KernelMutexUncontestedRelease(const void* mutex_id) {
    if constexpr (Level == KernelMutexTracingLevel::All) {
      KernelMutexTrace(TAG_KERNEL_MUTEX_RELEASE, mutex_id, nullptr, 0);
    }
  }

  void KernelMutexBlock(const void* mutex_id, const Thread* blocker, uint32_t waiter_count) {
    KernelMutexTrace(TAG_KERNEL_MUTEX_BLOCK, mutex_id, blocker, waiter_count);
  }

  void KernelMutexWake(const void* mutex_id, const Thread* new_owner, uint32_t waiter_count) {
    KernelMutexTrace(TAG_KERNEL_MUTEX_RELEASE, mutex_id, new_owner, waiter_count);
  }

 private:
  void KernelMutexTrace(uint32_t tag, const void* mutex_id, const Thread* t,
